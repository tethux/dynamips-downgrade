...
static const struct cisco_eeprom eeprom_c7200_io_fe = {
    "C7200-IO-FE",
    (m_uint16_t *)eeprom_c7200_io_fe_data,
...
static int dev_c7200_iocard_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  data = dev_dec21140_init(vm, card->dev_name, card->pci_bus,
...
static int dev_c7200_iocard_shutdown(vm_instance_t *vm,
...
static int dev_c7200_pa_fe_tx_shutdown(vm_instance_t *vm,
                                       struct cisco_card *card) {
  /* Remove the PA EEPROM */
  cisco_card_unset_eeprom(card);
  c7200_set_slot_eeprom(VM_C7200(vm), card->slot_id, NULL);
...
/* Bind a Network IO descriptor */
static int dev_c7200_pa_fe_tx_set_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id,
                                      netio_desc_t *nio) {
  struct dec21140_data *d = card->drv_info;
...
/* Unbind a Network IO descriptor */
static int dev_c7200_pa_fe_tx_unset_nio(vm_instance_t *vm,
                                        struct cisco_card *card,
                                        u_int port_id) {
  struct dec21140_data *d = card->drv_info;
...
static int dev_c7200_pa_i8254x_shutdown(vm_instance_t *vm,
                                        struct cisco_card *card) {
...
/* Bind a Network IO descriptor */
static int dev_c7200_pa_i8254x_set_nio(vm_instance_t *vm,
                                       struct cisco_card *card, u_int port_id,
                                       netio_desc_t *nio) {
...
/* Unbind a Network IO descriptor */
static int dev_c7200_pa_i8254x_unset_nio(vm_instance_t *vm,
                                         struct cisco_card *card,
                                         u_int port_id) {
  struct pa_i8254x_data *d = card->drv_info;
 ...
 */
static int dev_c7200_pa_2fe_tx_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] = dev_i8254x_init(vm, card->dev_name, 0, card->pci_bus, i,
...
static int dev_c7200_pa_ge_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  data->port[0] = dev_i8254x_init(vm, card->dev_name, 0, card->pci_bus, 0,
                                  c7200_net_irq_for_slot_port(slot, 0));
...
static const struct cisco_eeprom eeprom_c7200_io_2fe = {
    "C7200-IO-2FE",
    (m_uint16_t *)eeprom_c7200_io_2fe_data,
 ...
 */
static int dev_c7200_iocard_2fe_init(vm_instance_t *vm,
                                     struct cisco_card *card) {
  struct pa_i8254x_data *data;
  u_int slot = card->slot_id;
  ...
  data->port[0] = dev_i8254x_init(vm, card->dev_name, 0, card->pci_bus, 1,
                                  c7200_net_irq_for_slot_port(slot, 1));

  /* Port Fa0/1 is on PCI Device 0 */
  data->port[1] = dev_i8254x_init(vm, card->dev_name, 0, card->pci_bus, 0,
                                  c7200_net_irq_for_slot_port(slot, 0));
...
static const struct cisco_eeprom eeprom_c7200_io_ge_e = {
    "C7200-IO-GE-E",
    (m_uint16_t *)eeprom_c7200_io_ge_e_data,
 ...
 */
static int dev_c7200_iocard_ge_e_init(vm_instance_t *vm,
                                      struct cisco_card *card) {
  struct pa_i8254x_data *data;
  u_int slot = card->slot_id;
...
static int dev_c7200_pa_4e_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] = dev_am79c971_init(vm, card->dev_name,
                                      AM79C971_TYPE_10BASE_T, card->pci_bus, i,
                                      c7200_net_irq_for_slot_port(slot, i));
  }
...
static int dev_c7200_pa_4e8e_shutdown(vm_instance_t *vm,
                                      struct cisco_card *card) {
...
/* Bind a Network IO descriptor */
static int dev_c7200_pa_4e8e_set_nio(vm_instance_t *vm, struct cisco_card *card,
                                     u_int port_id, netio_desc_t *nio) {
...
/* Unbind a Network IO descriptor */
static int dev_c7200_pa_4e8e_unset_nio(vm_instance_t *vm,
                                       struct cisco_card *card, u_int port_id) {
  struct pa_4e8e_data *d = card->drv_info;
...
static int dev_c7200_npeg2_shutdown(vm_instance_t *vm,
                                    struct cisco_card *card) {
  /* Nothing to do */
  return (0);
}
...
static int dev_c7200_npeg2_unset_nio(vm_instance_t *vm, struct cisco_card *card,
                                     u_int port_id) {
  c7200_t *router = VM_C7200(vm);
  struct mv64460_data *mv_data = router->mv64460_sysctr;
