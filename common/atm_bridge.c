...
/* Receive an ATM cell */
static int atm_bridge_recv_cell(netio_desc_t *nio, u_char *atm_cell,
                                ssize_t cell_len, atm_bridge_t *t) {
  ...
  if ((status = atm_aal5_recv(&t->arc, atm_cell)) == 1) {
    ...
    if ((t->arc.len > ATM_RFC1483B_HLEN) &&
      ...
      netio_send(t->eth_nio, t->arc.buffer + ATM_RFC1483B_HLEN,
...
int atm_bridge_configure(atm_bridge_t *t, char *eth_nio, char *atm_nio,
  ...
  /* Add ATM RX listener */
  if (netio_rxl_add(t->atm_nio, (netio_rx_handler_t)atm_bridge_recv_cell, t,
                    NULL) == -1)
    goto error;

  /* Add Ethernet RX listener */
  if (netio_rxl_add(t->eth_nio, (netio_rx_handler_t)atm_bridge_recv_pkt, t,
                    NULL) == -1)
    goto error;
...
int atm_bridge_delete(char *name) {
  return (registry_delete_if_unused(name, OBJ_TYPE_ATM_BRIDGE, atm_bridge_free,
                                    NULL));
}
...
int atm_bridge_cfg_setup(atm_bridge_t *t, char **tokens, int count) {
  ...
  return (atm_bridge_configure(t, tokens[1], tokens[2], atoi(tokens[3]),
                               atoi(tokens[4])));
}
