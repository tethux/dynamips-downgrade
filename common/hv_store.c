...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (registry_rename(argv[0], newname, OBJ_TYPE_STORE)) {
    ...
    registry_unref(so->name, OBJ_TYPE_STORE);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename object '%s'", argv[0]);
    return (-1);
  }
  ...
  registry_unref(so->name, OBJ_TYPE_STORE);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "object '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static void cmd_show_obj_list(registry_entry_t *entry, void *opt, int *err) {
  ...
  hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "%s (len=%ld)", entry->name,
                        (long)so->len);
}
