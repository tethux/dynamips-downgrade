...
static void atm_send_fifo(struct atm_seg_context *asc) {
  if (!asc->txfifo_avail) {
    asc->aal5_crc = crc32_compute(
        ~asc->aal5_crc, &asc->txfifo_cell[ATM_HDR_SIZE], ATM_PAYLOAD_SIZE);
    atm_send_cell(asc);
...
static void atm_aal5_add_trailer(struct atm_seg_context *asc) {
  ...
  asc->aal5_crc = crc32_compute(~asc->aal5_crc, &asc->txfifo_cell[ATM_HDR_SIZE],
                                ATM_PAYLOAD_SIZE - 4);
...
int atm_aal5_send(netio_desc_t *nio, u_int vpi, u_int vci, struct iovec *iov,
                  int iovcnt) {
...
int atm_aal5_send_rfc1483b(netio_desc_t *nio, u_int vpi, u_int vci, void *pkt,
                           size_t len) {
  struct iovec vec[2];
