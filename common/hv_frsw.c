...
static int cmd_create(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!(t = frsw_create_table(argv[0]))) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to create frame-relay switch '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (registry_exists(argv[1], OBJ_TYPE_FRSW)) {
    frsw_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename FRSW '%s', '%s' already exists",
                          argv[0], argv[1]);
    return (-1);
  }
  ...
  if (!(newname = mp_strdup(&t->mp, argv[1]))) {
    frsw_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename FRSW '%s', out of memory", argv[0]);
    return (-1);
  }

  if (registry_rename(argv[0], newname, OBJ_TYPE_FRSW)) {
    mp_free(newname);
    frsw_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1, "unable to rename FRSW '%s'",
                          argv[0]);
    return (-1);
  }
  ...
  frsw_release(argv[1]);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "FRSW '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static int cmd_delete(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (res == 1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_DELETE, 1, "unable to delete FRSW '%s'",
                          argv[0]);
  }
