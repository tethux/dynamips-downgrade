...
static void show_usage(vm_instance_t *vm, int argc, char *argv[]) {
  ...
  printf(
      ...
      "  -C, --startup-config <file> : Import IOS configuration file into "
      "NVRAM\n"
      ...
      "\n",
      LOGFILE_DEFAULT_NAME, VM_TIMER_IRQ_CHECK_ITV, vm->ram_size, vm->rom_size,
      vm->nvram_size, vm->conf_reg_setup, vm->clock_divisor,
      vm->pcmcia_disk_size[0], vm->pcmcia_disk_size[1]);
...
static int parse_std_cmd_line(int argc, char *argv[]) {
  ...
  while ((option = getopt_long(argc, argv, options_list, cmd_line_lopts,
                               NULL)) != -1) {
    switch (option) {
    ...
    case 'U':
      ...
      if (vtty_parse_serial_option(&vm->vtty_con_serial_option, optarg)) {
        fprintf(stderr, "Invalid Console serial interface descriptor!\n");
        goto exit_failure;
      }
      break;
