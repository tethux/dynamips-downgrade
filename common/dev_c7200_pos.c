...
static void *dev_pos_access(cpu_gen_t *cpu, struct vdevice *dev,
                            ...
                            m_uint64_t *data) {
  _maybe_used struct pos_oc3_data *d = dev->priv_data;
 ...
 */
static void dev_pos_oc3_receive_pkt(struct pos_oc3_data *d, u_char *pkt,
                                    ssize_t pkt_len) {
...
/* Handle the RX ring */
static int dev_pos_oc3_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                     ssize_t pkt_len, struct pos_oc3_data *d) {
#if DEBUG_RECEIVE
 ...
 * pci_pos_read()
 */
static m_uint32_t pci_pos_read(cpu_gen_t *cpu, struct pci_device *dev,
                               int reg) {
  struct pos_oc3_data *d = dev->priv_data;
 ...
 * pci_pos_write()
 */
static void pci_pos_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                          m_uint32_t value) {
  struct pos_oc3_data *d = dev->priv_data;
...
int dev_c7200_pa_pos_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  if (!(d->rx_obj =
            dev_plx9060_init(vm, d->rx_name, card->pci_bus, 0, &d->rx_dev))) {
    goto err_rx_obj;
  }

  /* Initialize PLX9060 for TX part */
  if (!(d->tx_obj =
            dev_plx9060_init(vm, d->tx_name, card->pci_bus, 1, &d->tx_dev))) {
    goto err_tx_obj;
  }

  /* Initialize PLX9060 for CS part (CS=card status, chip status, ... ?) */
  if (!(d->cs_obj =
            dev_plx9060_init(vm, d->cs_name, card->pci_bus, 2, &d->cs_dev))) {
    goto err_cs_obj;
  }
