...
static int dev_c3745_nm_eth_init(vm_instance_t *vm, struct cisco_card *card,
  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] =
        dev_am79c971_init(vm, card->dev_name, interface_type, card->pci_bus, 0,
...
static int dev_c3745_nm_eth_shutdown(vm_instance_t *vm,
 ...
 */
static int dev_c3745_nm_1fe_tx_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
static int dev_c3745_nm_16esw_shutdown(vm_instance_t *vm,
                                       struct cisco_card *card) {
...
/* Bind a Network IO descriptor */
static int dev_c3745_nm_16esw_set_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id,
                                      netio_desc_t *nio) {
...
/* Unbind a Network IO descriptor */
static int dev_c3745_nm_16esw_unset_nio(vm_instance_t *vm,
                                        struct cisco_card *card,
                                        u_int port_id) {
  struct nm_16esw_data *d = card->drv_info;
...
/* Show debug info */
static int dev_c3745_nm_16esw_show_info(vm_instance_t *vm,
                                        struct cisco_card *card) {
  struct nm_16esw_data *d = card->drv_info;
...
static int dev_c3745_gt96100_fe_init(vm_instance_t *vm,
                                     struct cisco_card *card) {
  if (card->slot_id != 0) {
...
static int dev_c3745_gt96100_fe_shutdown(vm_instance_t *vm,
                                         struct cisco_card *card) {
  return (0);
}

/* Bind a Network IO descriptor */
static int dev_c3745_gt96100_fe_set_nio(vm_instance_t *vm,
                                        struct cisco_card *card, u_int port_id,
                                        netio_desc_t *nio) {
...
struct cisco_card_driver dev_c3745_nm_16esw_driver = {
    ...
    dev_c3745_nm_16esw_unset_nio,
    dev_c3745_nm_16esw_show_info,
};
