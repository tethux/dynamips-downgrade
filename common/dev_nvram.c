 ...
 * dev_nvram_access()
 */
void *dev_nvram_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
int generic_nvram_extract_config(vm_instance_t *vm, char *dev_name,
  ...
  if (file_size < nvram_offset + nvram_size) {
    vm_error(vm,
             "generic_nvram_extract_config: NVRAM filesystem doesn't fit "
             "inside the %s file!\n",
             dev_name);
    goto done;
  }
  ...
  // normal + backup
  fs = fs_nvram_open(base_ptr + nvram_offset, nvram_size, addr,
                     (format & FS_NVRAM_FORMAT_MASK));
  if (fs == NULL)
    goto err_errno;

  ret = fs_nvram_read_config(fs, startup_config, startup_len, private_config,
                             private_len);
  if (ret)
    goto err_ret;
...
int generic_nvram_push_config(vm_instance_t *vm, char *dev_name,
  ...
  if (file_size < nvram_offset + nvram_size) {
    vm_error(vm,
             "generic_nvram_push_config: NVRAM filesystem doesn't fit inside "
             "the %s file!\n",
             dev_name);
    return (-1);
  }
  ...
  fs = fs_nvram_open(base_ptr + nvram_offset, nvram_size, addr,
                     (format & FS_NVRAM_FORMAT_MASK) |
                         FS_NVRAM_FLAG_OPEN_CREATE);
  if (fs == NULL)
    goto err_errno;

  ret = fs_nvram_read_config(fs, &prev_startup_config, &prev_startup_len,
                             &prev_private_config, &prev_private_len);
  if (ret)
    goto err_ret;
  ...
  ret = fs_nvram_write_config(fs, startup_config, startup_len, private_config,
                              private_len);
  if (ret)
    goto err_ret;
