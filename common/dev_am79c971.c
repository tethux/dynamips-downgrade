...
_maybe_used static void mii_reg_write(struct am79c971_data *d, u_int phy,
                                      u_int reg, m_uint16_t value) {
  if ((phy < 32) && (reg < 32))
...
static void am79c971_rdp_access(cpu_gen_t *cpu, struct am79c971_data *d,
  ...
  case 88: /* CSR88: Chip ID Register Lower (VER=0, PARTID=0x2623, MANFID=1,
              ONE=1) */
    if (op_type == MTS_READ) {
      switch (d->type) {
 ...
 */
static int am79c971_receive_pkt(struct am79c971_data *d, u_char *pkt,
                                ssize_t pkt_len) {
...
#if DEBUG_RECEIVE
    AM79C971_LOG(d,
                 "am79c971_handle_rxring: i=%d, addr=0x%8.8x: "
                 "rmd[0]=0x%x, rmd[1]=0x%x, rmd[2]=0x%x, rmd[3]=0x%x\n",
                 i, rx_current, rxdc->rmd[0], rxdc->rmd[1], rxdc->rmd[2],
                 rxdc->rmd[3]);
#endif
...
/* Handle the RX ring */
static int am79c971_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                  ssize_t pkt_len, struct am79c971_data *d) {
  /*
...
#if DEBUG_TRANSMIT
  AM79C971_LOG(d,
               "am79c971_handle_txring: 1st desc: "
...
static m_uint32_t pci_am79c971_read(cpu_gen_t *cpu, struct pci_device *dev,
  ...
  */
  ...
  /* Base-Class, Sub-Class, Programming IF, Revision ID (read-only, should be
   * 0x02000021?) */
  case 0x08:
    return (0x02000002);
 ...
 */
static void pci_am79c971_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                               m_uint32_t value) {
  struct am79c971_data *d = dev->priv_data;
 ...
 */
struct am79c971_data *dev_am79c971_init(vm_instance_t *vm, char *name,
                                        int interface_type,
  ...
  /* Add as PCI device */
  pci_dev = pci_dev_add(pci_bus, name, AM79C971_PCI_VENDOR_ID,
                        AM79C971_PCI_PRODUCT_ID, pci_device, 0, irq, d, NULL,
                        pci_am79c971_read, pci_am79c971_write);
