  ...
  for (i = 0; i < MEMLOG_COUNT; i++) {
    ...
    if (cpu_get_pc(cpu)) {
      ...
      printf("CPU%u: pc=0x%8.8llx, vaddr=0x%8.8llx, "
             ...
             (acc->op_type == MTS_READ) ? "read " : "write", s_data);
    }
  }
}
...
void physmem_copy_to_vm(vm_instance_t *vm, void *real_buffer, m_uint64_t paddr,
                        size_t len) {
  m_uint64_t dummy;
  m_uint32_t r;
  u_char *ptr;
  ...
  while (len > 0) {
    ...
    if (likely(ptr != NULL)) {
      ...
      switch (r) {
      case 4:
        physmem_copy_u32_to_vm(vm, paddr, htovm32(*(m_uint32_t *)real_buffer));
        break;
      case 2:
        physmem_copy_u16_to_vm(vm, paddr, htovm16(*(m_uint16_t *)real_buffer));
        break;
      case 1:
...
void physmem_copy_u32_to_vm(vm_instance_t *vm, m_uint64_t paddr,
                            m_uint32_t val) {
  m_uint64_t tmp = val;
  m_uint32_t *ptr;
...
void physmem_copy_u16_to_vm(vm_instance_t *vm, m_uint64_t paddr,
                            m_uint16_t val) {
  m_uint64_t tmp = val;
  m_uint16_t *ptr;
...
void physmem_dump_vm(vm_instance_t *vm, m_uint64_t paddr,
                     m_uint32_t u32_count) {
  m_uint32_t i;

  for (i = 0; i < u32_count; i++) {
    vm_log(vm, "physmem_dump", "0x%8.8llx: 0x%8.8x\n", paddr + (i << 2),
           physmem_copy_u32_from_vm(vm, paddr + (i << 2)));
  }
}
