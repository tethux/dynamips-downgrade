...
int dev_c7200_pa_mc8te1_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  /* Create the PM7380 */
  d->pci_dev = pci_dev_add(card->pci_bus, card->dev_name, 0x11f8, 0x7380, 0, 0,
                           d->irq, d, NULL, pci_pos_read, pci_pos_write);
  ...
  if (!(d->plx_obj = dev_plx9054_init(vm, d->plx_name, card->pci_bus, 1,
                                      &d->ssram_dev, NULL))) {
    goto err_plx_obj;
  }

  /* Set callback function for PLX9054 PCI-To-Local doorbell */
  dev_plx_set_pci2loc_doorbell_cbk(
      d->plx_obj->data, (dev_plx_doorbell_cbk)plx9054_doorbell_callback, d);
