...
static void dot1q_push_tag(m_uint8_t *pkt, ethsw_packet_t *sp, u_int vlan,
  ...
  memcpy(pkt + sizeof(n_eth_dot1q_hdr_t), sp->pkt + (N_ETH_HLEN - 2),
...
static void dot1q_pop_tag(m_uint8_t *pkt, ethsw_packet_t *sp) {
  ...
  memcpy(pkt + (N_ETH_HLEN - 2), sp->pkt + sizeof(n_eth_dot1q_hdr_t),
...
static inline int ethsw_receive(ethsw_table_t *t, netio_desc_t *nio,
  ...
  case ETHSW_PORT_TYPE_DOT1Q:
    ...
    if (ethertype != N_ETH_PROTO_DOT1Q && ethertype != N_ETH_PROTO_DOT1Q_2 &&
        ethertype != N_ETH_PROTO_DOT1Q_3 && ethertype != N_ETH_PROTO_DOT1Q_4) {
      sp.input_vlan = nio->vlan_id;
      sp.input_tag = FALSE;
    } else {
  ...
  case ETHSW_PORT_TYPE_ISL:
    ...
    /* Get the VLAN id */
    isl_hdr = PTR_ADJUST(n_eth_isl_hdr_t *, llc_hdr, sizeof(n_eth_llc_hdr_t));
...
int ethsw_release(char *name) { return (registry_unref(name, OBJ_TYPE_ETHSW)); }
...
static void ethsw_reg_save_config(registry_entry_t *entry, void *opt,
                                  int *err) {
...
static int ethsw_cfg_set_access_port(ethsw_table_t *t, char **tokens,
                                     int count) {
...
/* Set a port as a 802.1q trunk port */
static int ethsw_cfg_set_dot1q_port(ethsw_table_t *t, char **tokens,
                                    int count) {
