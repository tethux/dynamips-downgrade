...
static inline void
dev_c1700_iofpga_net_update_irq(struct c1700_iofpga_data *d) {
  if (d->net_irq_status) {
...
void dev_c1700_iofpga_net_set_irq(struct c1700_iofpga_data *d, u_int slot,
                                  u_int port) {
#if DEBUG_NET_IRQ
...
void dev_c1700_iofpga_net_clear_irq(struct c1700_iofpga_data *d, u_int slot,
                                    u_int port) {
#if DEBUG_NET_IRQ
 ...
 */
static void *dev_c1700_iofpga_access(cpu_gen_t *cpu, struct vdevice *dev,
...
static void dev_c1700_iofpga_shutdown(vm_instance_t *vm,
...
int dev_c1700_iofpga_init(c1700_t *router, m_uint64_t paddr, m_uint32_t len) {
  ...
  mpc860_spi_set_tx_callback(router->mpc_data, dev_c1700_mpc860_spi_tx_callback,
                             d);
