...
#define GT_SDMA_CMD_OFFSET                                                     \
  offsetof(struct sdma_desc, cmd_stat) /* Offset of the Command/Status word */
...
#if DEBUG_DMA
      vm_log(
          vm, "GT_DMA", "starting transfer from 0x%x to 0x%x (size=%u bytes)\n",
...
static int gt_dma_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
static void gt_sdma_update_channel_int_status(struct gt_data *d,
                                              u_int chan_id) {
  m_uint32_t ch_st;
...
static int gt_sdma_tx_start(struct gt_data *d, struct sdma_channel *chan) {
  ...
  for (;;) {
    ...
    if (!(ptxd->cmd_stat & GT_TXDESC_F)) {
      ...
      physmem_copy_u32_to_vm(d->vm, tx_current + GT_SDMA_CMD_OFFSET,
                             ptxd->cmd_stat);
    }
...
static int gt_sdma_handle_rxqueue(struct gt_data *d,
                                  struct sdma_channel *channel, u_char *pkt,
                                  ssize_t pkt_len) {
...
#if DEBUG_SDMA
  GT_LOG(d,
         "SDMA channel %u: reading desc at 0x%8.8x "
         "[buf_size=0x%8.8x,cmd_stat=0x%8.8x,"
         "next_ptr=0x%8.8x,buf_ptr=0x%8.8x]\n",
         channel->id, rx_start, rxd0.buf_size, rxd0.cmd_stat, rxd0.next_ptr,
         rxd0.buf_ptr);
#endif
...
static int gt_sdma_handle_rx_pkt(netio_desc_t *nio, u_char *pkt,
                                 ssize_t pkt_len, struct gt_data *d,
                                 void *arg) {
...
int dev_gt96100_mpsc_set_nio(struct gt_data *d, u_int chan_id,
  ...
  channel->nio = nio;
  netio_rxl_add(nio, (netio_rx_handler_t)gt_sdma_handle_rx_pkt, d,
                (void *)(u_long)chan_id);
  return (0);
}
...
static int gt_eth_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
                         u_int op_size, u_int op_type, m_uint64_t *data) {
  ...
  case 0x88808:
    ...
    cpu_log(cpu, "GT96100/ETH",
            "PCXR: Port Configuration Extend Register [%s]\n", access);
#endif
    if (op_type == MTS_READ) {
      *data = port->pcxr;
      *data |= GT_PCXR_SPEED;
...
#if DEBUG_ETH
    cpu_log(cpu, "GT96100/ETH",
            "Ethernet MIB Counters - Frames Received [%s]\n", access);
#endif
    if (op_type == MTS_READ) {
      *data = port->rx_frames;
      port->rx_frames = 0;
...
/* Inverse a nibble */
static const int inv_nibble[16] = {0x0, 0x8, 0x4, 0xC, 0x2, 0xA, 0x6, 0xE,
...
#if DEBUG_ETH_HASH
  GT_LOG(d,
         "Hash Lookup for Ethernet address "
         "%2.2x:%2.2x:%2.2x:%2.2x:%2.2x:%2.2x: addr=0x%x\n",
         addr->eth_addr_byte[0], addr->eth_addr_byte[1], addr->eth_addr_byte[2],
         addr->eth_addr_byte[3], addr->eth_addr_byte[4], addr->eth_addr_byte[5],
         hte_addr);
#endif
...
static inline int gt_eth_handle_rx_daddr(struct gt_data *d,
                                         struct eth_port *port, u_int hash_res,
...
#if DEBUG_ETH_RX
  GT_LOG(d,
         "port %u/queue %u: reading desc at 0x%8.8x "
         "[buf_size=0x%8.8x,cmd_stat=0x%8.8x,"
         "next_ptr=0x%8.8x,buf_ptr=0x%8.8x]\n",
         port_id, queue, rx_start, rxd0.buf_size, rxd0.cmd_stat, rxd0.next_ptr,
         rxd0.buf_ptr);
#endif
...
static int gt_eth_handle_rx_pkt(netio_desc_t *nio, u_char *pkt, ssize_t pkt_len,
...
int dev_gt64010_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
                     m_uint32_t len, u_int irq) {
  struct gt_data *d;
  ...
  if (!pci_dev_lookup(d->bus[0], 0, 0, 0)) {
    d->pci_dev = pci_dev_add(d->bus[0], name, PCI_VENDOR_GALILEO,
                             PCI_PRODUCT_GALILEO_GT64010, 0, 0, irq, d, NULL,
                             NULL, NULL);
...
int dev_gt64120_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
                     m_uint32_t len, u_int irq) {
  struct gt_data *d;
  ...
  if (!pci_dev_lookup(d->bus[0], 0, 0, 0)) {
    d->pci_dev = pci_dev_add(d->bus[0], name, PCI_VENDOR_GALILEO,
                             PCI_PRODUCT_GALILEO_GT64120, 0, 0, irq, d, NULL,
                             pci_gt64120_read, NULL);
    if (!d->pci_dev) {
...
int dev_gt96100_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
                     m_uint32_t len, u_int int0_irq, u_int int1_irq,
  ...
  if (!pci_dev_lookup(d->bus[0], 0, 0, 0)) {
    d->pci_dev = pci_dev_add(d->bus[0], name, PCI_VENDOR_GALILEO,
                             PCI_PRODUCT_GALILEO_GT96100, 0, 0, -1, d, NULL,
                             pci_gt96100_read, NULL);
    if (!d->pci_dev) {
...
int dev_gt96100_eth_set_nio(struct gt_data *d, u_int port_id,
                            netio_desc_t *nio) {
  struct eth_port *port;
  ...
  port->nio = nio;
  netio_rxl_add(nio, (netio_rx_handler_t)gt_eth_handle_rx_pkt, d,
                (void *)(u_long)port_id);
  return (0);
}
