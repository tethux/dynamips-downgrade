...
static m_uint32_t channel_offset[MUESLIX_NR_CHANNELS] = {
    MUESLIX_CHANNEL0_OFFSET,
    MUESLIX_CHANNEL1_OFFSET,
    MUESLIX_CHANNEL2_OFFSET,
    MUESLIX_CHANNEL3_OFFSET,
};
...
static inline int dev_mueslix_is_rx_tx_enabled(struct mueslix_data *d,
                                               u_int id) {
      ...
      *data = channel->crc_ctrl_reg;
      ...
      default:
        MUESLIX_LOG(channel->parent,
                    "channel %u: unknown value "
                    "for CRC ctrl reg 0x%4.4x\n",
                    channel->id, channel->crc_ctrl_reg);

        channel->crc_size = 2;
      }
      MUESLIX_LOG(channel->parent, "channel %u: CRC size set to 0x%4.4x\n",
...
void *dev_mueslix_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
  ...
  if (op_type == MTS_READ) {
    ...
    cpu_log(cpu, d->name,
            "write access to offset=0x%x, pc=0x%llx, "
            "val=0x%llx, size=%u\n",
  ...
  for (i = 0; i < MUESLIX_NR_CHANNELS; i++)
    if ((offset >= channel_offset[i]) &&
      ...
      MUESLIX_LOCK(d);
      dev_mueslix_chan_access(cpu, &d->channel[i], offset - channel_offset[i],
...
#if DEBUG_ACCESS
      cpu_log(cpu, d->name, "channel_enable_mask = 0x%5.5llx at pc=0x%llx\n",
              *data, cpu_get_pc(cpu));
#endif
...
static int dev_mueslix_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                     ssize_t pkt_len,
                                     struct mueslix_channel *channel) {
  struct mueslix_data *d = channel->parent;
...
#if DEBUG_TRANSMIT
  MUESLIX_LOG(d,
              "mueslix_handle_txring: 1st desc: "
...
/* pci_mueslix_write() */
static void pci_mueslix_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                              m_uint32_t value) {
  struct mueslix_data *d = dev->priv_data;
...
struct mueslix_data *dev_mueslix_init(vm_instance_t *vm, char *name,
  ...
  /* Add as PCI device */
  pci_dev = pci_dev_add(pci_bus, name, MUESLIX_PCI_VENDOR_ID,
                        MUESLIX_PCI_PRODUCT_ID, pci_device, 0, irq, d, NULL,
                        pci_mueslix_read, pci_mueslix_write);
...
int dev_mueslix_set_nio(struct mueslix_data *d, u_int channel_id,
  ...
  channel->nio = nio;
  channel->tx_tid =
      ptask_add((ptask_callback)dev_mueslix_handle_txring, channel, NULL);
  netio_rxl_add(nio, (netio_rx_handler_t)dev_mueslix_handle_rxring, channel,
                NULL);
  return (0);
}
