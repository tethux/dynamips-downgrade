...
/* DLCI lookup */
frsw_conn_t *frsw_dlci_lookup(frsw_table_t *t, netio_desc_t *input,
                              u_int dlci) {
  frsw_conn_t *vc;
...
ssize_t frsw_handle_pkt(frsw_table_t *t, netio_desc_t *input, m_uint8_t *pkt,
                        ssize_t len) {
...
int frsw_create_vc(frsw_table_t *t, char *nio_input, u_int dlci_in,
  ...
  if (frsw_dlci_lookup(t, vc->input, dlci_in)) {
    fprintf(stderr,
            "FRSW %s: switching for VC %u on IF %s "
            "already defined.\n",
  ...
  if (netio_rxl_add(vc->input, (netio_rx_handler_t)frsw_recv_pkt, t, NULL) ==
      -1)
    goto error;
...
void frsw_save_config(frsw_table_t *t, FILE *fd) {
  ...
  for (i = 0; i < FRSW_HASH_SIZE; i++) {
    for (vc = t->dlci_table[i]; vc; vc = vc->next) {
      fprintf(fd, "frsw create_vc %s %s %u %s %u\n", t->name, vc->input->name,
              vc->dlci_in, vc->output->name, vc->dlci_out);
    }
  }
...
int frsw_cfg_create_if(frsw_table_t *t, char **tokens, int count) {
  ...
  case NETIO_TYPE_UNIX:
    if (count != 5) {
      fprintf(stderr,
              "FRSW: invalid number of arguments "
              "for UNIX NIO '%s'\n",
              tokens[1]);
      break;
    }
  ...
  if (!nio) {
    fprintf(stderr,
            "FRSW: unable to create NETIO descriptor of "
            "interface %s\n",
            tokens[1]);
    return (-1);
  }
...
int frsw_cfg_create_vc(frsw_table_t *t, char **tokens, int count) {
  ...
  return (frsw_create_vc(t, tokens[1], atoi(tokens[2]), tokens[3],
                         atoi(tokens[4])));
}
