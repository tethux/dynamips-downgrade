...
struct insn_lookup {
  m_uint32_t
      crc32_insn; /* crc32 of the mask and value of all the instructions */
...
/* Instruction lookup */
static forced_inline int ilt_get_index(rfc_array_t *a1, rfc_array_t *a2, int i1,
                                       int i2) {
  return ((a1->eqID[i1] * a2->nr_eqid) + a2->eqID[i2]);
}

static forced_inline int ilt_get_idx(insn_lookup_t *ilt, int a1, int a2, int i1,
                                     int i2) {
...
insn_lookup_t *ilt_create(char *table_name, m_uint32_t crc32, int nr_insn,
                          ilt_get_insn_cbk_t get_insn, ilt_check_cbk_t chk_lo,
                          ilt_check_cbk_t chk_hi);
