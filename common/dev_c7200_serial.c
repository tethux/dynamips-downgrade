...
static int dev_c7200_pa_4t_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  data = dev_mueslix_init(vm, card->dev_name, 1, card->pci_bus, 0,
...
static int dev_c7200_pa_4t_shutdown(vm_instance_t *vm,
...
static int dev_c7200_pa_8t_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  data->mueslix[0] = dev_mueslix_init(vm, card->dev_name, 1, card->pci_bus, 0,
                                      c7200_net_irq_for_slot_port(slot, 0));
  if (!data->mueslix[0]) {
    free(data);
    return (-1);
  ...
  data->mueslix[1] = dev_mueslix_init(vm, card->dev_name, 1, card->pci_bus, 1,
...
static int dev_c7200_pa_8t_shutdown(vm_instance_t *vm,
                                    struct cisco_card *card) {
