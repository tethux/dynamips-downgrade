  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] =
        dev_am79c971_init(vm, card->dev_name, interface_type, card->pci_bus,
                          i + (slot * 4), c2600_net_irq_for_slot_port(slot, i));
  }
...
static int dev_c2600_nm_eth_shutdown(vm_instance_t *vm,
...
/* ====================================================================== */
static int dev_c2600_mb1fe_eth_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
/* ====================================================================== */
static int dev_c2600_mb2fe_eth_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
 ...
 */
static int dev_c2600_nm_1fe_tx_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
static int dev_c2600_nm_16esw_shutdown(vm_instance_t *vm,
                                       struct cisco_card *card) {
...
/* Bind a Network IO descriptor */
static int dev_c2600_nm_16esw_set_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id,
                                      netio_desc_t *nio) {
...
/* Unbind a Network IO descriptor */
static int dev_c2600_nm_16esw_unset_nio(vm_instance_t *vm,
                                        struct cisco_card *card,
                                        u_int port_id) {
  struct nm_16esw_data *d = card->drv_info;
...
/* Show debug info */
static int dev_c2600_nm_16esw_show_info(vm_instance_t *vm,
                                        struct cisco_card *card) {
  struct nm_16esw_data *d = card->drv_info;
