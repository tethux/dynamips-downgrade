...
static void plx_map_space(struct plx_data *d, u_int id) {
  ...
  PLX_LOG(d, "device %u mapped at 0x%llx, size=0x%x\n", id,
          lspc->dev->phys_addr, lspc->dev->phys_len);
}

/* PLX device common access routine */
void *dev_plx_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
void *dev_plx9054_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
                         u_int op_size, u_int op_type, m_uint64_t *data) {
  struct plx_data *d = dev->priv_data;
 ...
 */
static m_uint32_t pci_plx_read(cpu_gen_t *cpu, struct pci_device *dev,
                               int reg) {
  struct plx_data *d = dev->priv_data;
 ...
 */
static void pci_plx_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                          m_uint32_t value) {
  struct plx_data *d = dev->priv_data;
 ...
 * pci_plx9054_write()
 */
static void pci_plx9054_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                              m_uint32_t value) {
  struct plx_data *d = dev->priv_data;
...
vm_obj_t *dev_plx9060_init(vm_instance_t *vm, char *name,
  ...
  d->pci_plx_dev =
      pci_dev_add(pci_bus, name, PLX_PCI_VENDOR_ID, PLX9060_PCI_PRODUCT_ID,
                  pci_device, 0, -1, d, NULL, pci_plx_read, pci_plx_write);
...
vm_obj_t *dev_plx9054_init(vm_instance_t *vm, char *name,
  ...
  /* Add PLX as a PCI device */
  d->pci_plx_dev = pci_dev_add(pci_bus, name, PLX_PCI_VENDOR_ID,
                               PLX9054_PCI_PRODUCT_ID, pci_device, 0, -1, d,
...
void dev_plx_set_pci2loc_doorbell_cbk(struct plx_data *d,
                                      dev_plx_doorbell_cbk cbk, void *arg) {
  if (d != NULL) {
