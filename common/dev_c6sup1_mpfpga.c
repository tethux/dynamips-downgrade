...
static inline void
dev_c6sup1_mpfpga_net_update_irq(struct c6sup1_mpfpga_data *d) {
  if (d->irq_status) {
...
void dev_c6sup1_mpfpga_net_set_irq(struct c6sup1_mpfpga_data *d, u_int slot,
                                   u_int port) {
#if DEBUG_NET_IRQ
...
void dev_c6sup1_mpfpga_net_clear_irq(struct c6sup1_mpfpga_data *d, u_int slot,
                                     u_int port) {
#if DEBUG_NET_IRQ
...
void *dev_c6sup1_mpfpga_access(cpu_gen_t *cpu, struct vdevice *dev,
  ...
  if (op_type == MTS_READ) {
    cpu_log(cpu, "MP_FPGA",
            ...
            cpu_get_pc(cpu), CPU_MIPS64(cpu)->gpr[MIPS_GPR_RA], op_size);
  } else {
    cpu_log(cpu, "MP_FPGA",
            "writing reg 0x%x at pc=0x%llx, ra=0x%llx "
            "data=0x%llx (size=%u)\n",
            offset, cpu_get_pc(cpu), CPU_MIPS64(cpu)->gpr[MIPS_GPR_RA], *data,
            op_size);
  }
#endif
        ...
        *data = SLOT_NOT_PRESENT;
        ...
        if (grp->eeprom[0] && grp->eeprom[0]->data) {
          *data = 0;
          ...
          if (d->slot_status[slot - 1] || (slot == d->router->sup_slot))
            *data |= SLOT_POWER_OK;
        }
        break;
...
static void dev_c6sup1_mpfpga_shutdown(vm_instance_t *vm,
...
void c6sup1_init_eeprom_groups(c6sup1_t *router) {
  ...
  grp->eeprom[0] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-CHASSIS-6509"));
  grp->eeprom[2] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-POWER-1000W"));
  grp->eeprom[3] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-POWER-1000W"));
  grp->eeprom[6] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-VTT"));
  grp->eeprom[7] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-VTT"));
  grp->eeprom[8] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-VTT"));
  ...
  grp->eeprom[0] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-SUP-SUP1A-2GE"));
  grp->eeprom[1] = buf;
  cisco_eeprom_copy(buf++, cisco_eeprom_find_c6k("C6K-EARL-PFC1"));
