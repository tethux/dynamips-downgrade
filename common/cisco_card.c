...
static void cisco_card_remove_all_nio_bindings(vm_instance_t *vm,
                                               struct cisco_card *card) {
...
static inline void cisco_card_enable_all_nio(vm_instance_t *vm,
                                             struct cisco_card *card) {
  struct cisco_nio_binding *nb;
...
static inline void cisco_card_disable_all_nio(vm_instance_t *vm,
                                              struct cisco_card *card) {
  struct cisco_nio_binding *nb;
...
/* Initialize a card */
static inline int cisco_card_init(vm_instance_t *vm, struct cisco_card *card,
                                  u_int id) {
  size_t len;
...
static u_int vm_slot_translate_port_id(vm_instance_t *vm, u_int slot_id,
                                       u_int port_id, struct cisco_card **rc) {
...
int vm_slot_set_flag(vm_instance_t *vm, u_int slot_id, u_int port_id,
                     u_int flag) {
  struct cisco_card **rc;
  u_int real_port_id;
...
/* Add a slot binding */
int vm_slot_add_binding(vm_instance_t *vm, char *dev_type, u_int slot_id,
                        u_int port_id) {
  ...
  if (!card || (card == *rc)) {
    ...
    if (card->driver->card_get_sub_info(vm, card, port_id, &drv_array,
                                        &card_type) == -1) {
...
int vm_slot_remove_nio_binding(vm_instance_t *vm, u_int slot_id,
                               u_int port_id) {
  struct cisco_nio_binding *nb;
  struct cisco_card *card, *rc;
  u_int real_port_id;
...
int vm_slot_enable_nio(vm_instance_t *vm, u_int slot_id, u_int port_id) {
  ...
  return (rc->driver->card_set_nio(vm, rc, real_port_id, nb->nio));
}
...
int vm_slot_disable_nio(vm_instance_t *vm, u_int slot_id, u_int port_id) {
  ...
  return (rc->driver->card_unset_nio(vm, rc, real_port_id));
}
...
struct cisco_eeprom *vm_slot_get_eeprom(vm_instance_t *vm, u_int slot_id,
                                        u_int port_id) {
  struct cisco_card *card, *rc;
...
int vm_slot_cmd_add_nio(vm_instance_t *vm, char *str) {
  ...
  snprintf(nio_name, sizeof(nio_name), "%s-i%u/%u/%u", vm_get_type(vm),
           vm->instance_id, slot_id, port_id);
  ...
  case NETIO_TYPE_UDP:
    ...
    nio = netio_desc_create_udp(nio_name, atoi(tokens[3]), tokens[4],
                                atoi(tokens[5]));
    break;
  ...
  case NETIO_TYPE_GEN_ETH:
    if (count != 4) {
      vm_error(vm, "invalid number of arguments for Generic Eth NIO '%s'\n",
               str);
      goto done;
    }
