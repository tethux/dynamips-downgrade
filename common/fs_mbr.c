...
void mbr_get_chs(m_uint8_t chs[3], m_uint16_t *cyl, m_uint8_t *head,
                 m_uint8_t *sect) {
  if (head)
    *head = chs[0];
  if (sect)
    *sect = (chs[1] & 0x3F);
  if (cyl)
    *cyl = ((m_uint16_t)(chs[1] & 0xC0) << 2) | (chs[2]);
}

/** Encode a CHS reference */
void mbr_set_chs(m_uint8_t chs[3], m_uint16_t cyl, m_uint8_t head,
                 m_uint8_t sect) {
