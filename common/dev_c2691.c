...
static int c2691_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(
      vm, "rom", C2691_NVRAM_OFFSET, C2691_NVRAM_SIZE, 0,
...
static int c2691_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                   size_t startup_len, u_char *private_config,
                                   size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(vm, "rom", vm->rom_size * 1048576,
                                  C2691_NVRAM_OFFSET, C2691_NVRAM_SIZE, 0,
...
static inline void c2691_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
static int c2691_burn_mac_addr(c2691_t *router, n_eth_addr_t *addr) {
  ...
  default:
    vm_error(router->vm,
             "c2691_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static int c2691_init_gt96100(c2691_t *router) {
  ...
  if (dev_gt96100_init(vm, "gt96100", C2691_GT96K_ADDR, 0x200000,
                       C2691_GT96K_IRQ, C2691_EXT_IRQ,
                       c2691_net_irq_for_slot_port(0, 0), 255) == -1)
    return (-1);
...
static int c2691_init_platform(c2691_t *router) {
  ...
  if (!(obj =
            dev_flash_init(vm, "rom", C2691_ROM_ADDR, vm->rom_size * 1048576)))
    goto err;
  ...
  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C2691_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
...
/* Get MAC address MSB */
static u_int c2691_get_mac_addr_msb(void) { return (0xC0); }
