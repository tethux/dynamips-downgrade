...
static int c3600_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(vm, "nvram", vm->nvram_rom_space, 0, 0,
                                     FS_NVRAM_FORMAT_DEFAULT, startup_config,
...
static int c3600_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                   size_t startup_len, u_char *private_config,
                                   size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(vm, "nvram", vm->nvram_size * 1024,
                                  vm->nvram_rom_space, 0, 0,
                                  FS_NVRAM_FORMAT_DEFAULT, startup_config,
                                  startup_len, private_config, private_len);
...
static inline void c3600_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
static int c3600_burn_mac_addr(c3600_t *router, n_eth_addr_t *addr) {
  ...
  default:
    vm_error(router->vm,
             "c3600_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static int c3600_init_platform(c3600_t *router) {
  ...
  /* NVRAM and calendar */
  dev_nvram_init(vm, "nvram", C3600_NVRAM_ADDR, vm->nvram_size * 1024,
                 &vm->conf_reg);
  ...
  if (!vm->rom_filename) {
    ...
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C3600_ROM_ADDR,
                 vm->rom_size * 1048576);
  }
...
/* Get MAC address MSB */
static u_int c3600_get_mac_addr_msb(void) { return (0xCC); }
