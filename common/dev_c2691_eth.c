...
static int dev_c2691_nm_eth_init(vm_instance_t *vm, struct cisco_card *card,
  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] =
        dev_am79c971_init(vm, card->dev_name, interface_type, card->pci_bus,
                          6 + i, c2691_net_irq_for_slot_port(slot, i));
  }
...
static int dev_c2691_nm_eth_shutdown(vm_instance_t *vm,
 ...
 */
static int dev_c2691_nm_1fe_tx_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
static int dev_c2691_nm_16esw_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  /* Create the device */
  data = dev_nm_16esw_init(vm, card->dev_name, slot, card->pci_bus, 6,
...
static int dev_c2691_nm_16esw_shutdown(vm_instance_t *vm,
                                       struct cisco_card *card) {
...
/* Bind a Network IO descriptor */
static int dev_c2691_nm_16esw_set_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id,
                                      netio_desc_t *nio) {
...
/* Show debug info */
static int dev_c2691_nm_16esw_show_info(vm_instance_t *vm,
                                        struct cisco_card *card) {
  struct nm_16esw_data *d = card->drv_info;
...
static int dev_c2691_gt96100_fe_init(vm_instance_t *vm,
                                     struct cisco_card *card) {
  if (card->slot_id != 0) {
...
static int dev_c2691_gt96100_fe_shutdown(vm_instance_t *vm,
                                         struct cisco_card *card) {
  return (0);
}

/* Bind a Network IO descriptor */
static int dev_c2691_gt96100_fe_set_nio(vm_instance_t *vm,
                                        struct cisco_card *card, u_int port_id,
                                        netio_desc_t *nio) {
