...
_maybe_used static char *pci_cfgreg_name(int reg) {
  static char *name[] = {"FID", "FCS", "FRV", "FLT", "BIO", "BMA",
                         "?",   "?",   "?",   "?",   "?",   "?",
                         "?",   "?",   "?",   "FIT", "FDA"};
...
void *dev_dec21140_access(cpu_gen_t *cpu, struct vdevice *dev,
  ...
  if (op_type == MTS_READ) {
    ...
    case 9:
      ...
      if ((*data & ~DEC21140_CSR9_TX_BIT) ==
          (DEC21140_CSR9_MII_READ | DEC21140_CSR9_READ |
 ...
 */
static int dev_dec21140_receive_pkt(struct dec21140_data *d, u_char *pkt,
                                    ssize_t pkt_len) {
...
static int dev_dec21140_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                      ssize_t pkt_len,
...
#if DEBUG_TRANSMIT
  DEC21140_LOG(d,
               "dec21140_handle_txring: 1st desc: "
 ...
 */
static void pci_dec21140_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                               m_uint32_t value) {
  struct dec21140_data *d = dev->priv_data;
...
struct dec21140_data *dev_dec21140_init(vm_instance_t *vm, char *name,
  ...
  /* Add as PCI device */
  pci_dev = pci_dev_add(pci_bus, name, DEC21140_PCI_VENDOR_ID,
                        DEC21140_PCI_PRODUCT_ID, pci_device, 0, irq, d, NULL,
                        pci_dec21140_read, pci_dec21140_write);
