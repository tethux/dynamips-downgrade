...
/* Add a breakpoint */
static int cmd_add_cpu_breakpoint(hypervisor_conn_t *conn, int argc,
                                  char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
  m_uint64_t addr;
...
/* Remove a breakpoint */
static int cmd_remove_cpu_breakpoint(hypervisor_conn_t *conn, int argc,
                                     char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
  m_uint64_t addr;
...
static int cmd_pmem_cfind(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (len == 0 || len % 2 == 1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_INV_PARAM, 1,
                          "Invalid byte sequence (invalid length=%u)",
                          (unsigned int)len);
    return (-1);
  }
  ...
  if (len != hex_decode(bytes, (unsigned char *)argv[2], len)) {
    ...
    vm_release(vm);
    hypervisor_send_reply(conn, HSC_ERR_INV_PARAM, 1,
                          "Invalid byte sequence (non-hexadecimal character)");
    return (-1);
  }
