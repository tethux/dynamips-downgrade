...
static int cmd_create_udp(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!nio) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1, "unable to create UDP NIO");
    return (-1);
  }
 ...
 */
static int cmd_create_udp_auto(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  netio_desc_t *nio;
  int local_port;

  nio = netio_desc_create_udp_auto(argv[0], argv[1], atoi(argv[2]),
                                   atoi(argv[3]));
 ...
 */
static int cmd_connect_udp_auto(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  netio_desc_t *nio;
  int res;
 ...
 */
#ifdef LINUX_ETH
static int cmd_create_linux_eth(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  netio_desc_t *nio;
...
static int cmd_create_null(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!nio) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1, "unable to create Null NIO");
    return (-1);
  }
...
static int cmd_create_fifo(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!nio) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1, "unable to create FIFO NIO");
    return (-1);
  }
 ...
 */
static int cmd_crossconnect_fifo(hypervisor_conn_t *conn, int argc,
                                 char *argv[]) {
  netio_desc_t *a, *b;
...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!(newname = strdup(argv[1]))) {
    netio_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename NIO '%s', out of memory", argv[0]);
    return (-1);
  }
  ...
  if (registry_rename(argv[0], newname, OBJ_TYPE_NIO)) {
    free(newname);
    netio_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1, "unable to rename NIO '%s'",
                          argv[0]);
    return (-1);
  }
  ...
  netio_release(argv[1]);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "NIO '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static int cmd_delete(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (res == 1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_DELETE, 1, "unable to delete NIO '%s'",
                          argv[0]);
  }
...
static int cmd_bind_filter(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!res) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_UNK_OBJ, 1, "Unknown filter %s",
                          argv[2]);
  }
  return (0);
}
...
static int cmd_setup_filter(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!res) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_UNSPECIFIED, 1,
...
static int cmd_get_stats(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "%llu %llu %llu %llu", spi, spo,
                        sbi, sbo);
  return (0);
}
