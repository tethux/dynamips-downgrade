...
struct vdevice *dev_lookup(vm_instance_t *vm, m_uint64_t phys_addr,
                           int cached) {
  struct vdevice *dev;
...
struct vdevice *dev_create(char *name) {
  ...
  if (!(dev = malloc(sizeof(*dev)))) {
    fprintf(stderr,
            "dev_create: insufficient memory to "
            "create device '%s'.\n",
            name);
    return NULL;
  }
...
void dev_remove(vm_instance_t *vm, struct vdevice *dev) {
  ...
  vm_log(vm, "DEVICE",
         ...
         dev->fd, (m_uint64_t)dev->host_addr, dev->flags);
...
struct vdevice *dev_create_ram_alias(vm_instance_t *vm, char *name, char *orig,
  ...
  if (!(dev = dev_remap(name, orig_dev, paddr, len))) {
    fprintf(stderr,
            "VM%u: dev_create_ram_alias: unable to create "
            "new device %s.\n",
            vm->instance_id, name);
    return NULL;
  }
