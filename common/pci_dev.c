...
struct pci_device *pci_dev_lookup_local(struct pci_bus *pci_bus, int device,
                                        int function) {
...
/* PCI Device lookup */
struct pci_device *pci_dev_lookup(struct pci_bus *pci_bus_root, int bus,
...
void pci_dev_data_handler(cpu_gen_t *cpu, struct pci_bus *pci_bus,
  ...
  if (!dev) {
    if (op_type == MTS_READ) {
      cpu_log(cpu, "PCI",
              "read request for unknown device at pc=0x%llx "
              "(bus=%d,device=%d,function=%d,reg=0x%2.2x).\n",
              cpu_get_pc(cpu), bus, device, function, reg);
    } else {
      cpu_log(cpu, "PCI",
              "write request (data=0x%8.8x) for unknown device "
              "at pc=0x%llx (bus=%d,device=%d,function=%d,reg=0x%2.2x).\n",
              pci_swap(*data, swap), cpu_get_pc(cpu), bus, device, function,
              reg);
    }
    ...
    if (op_type == MTS_READ) {
      cpu_log(cpu, "PCI",
              "read request for device '%s' at pc=0x%llx: "
              "bus=%d,device=%d,function=%d,reg=0x%2.2x\n",
              dev->name, cpu_get_pc(cpu), bus, device, function, reg);
    } else {
      cpu_log(cpu, "PCI",
              "write request (data=0x%8.8x) for device '%s' at pc=0x%llx: "
              "bus=%d,device=%d,function=%d,reg=0x%2.2x\n",
              pci_swap(*data, swap), dev->name, cpu_get_pc(cpu), bus, device,
              function, reg);
    }
#endif
    if (op_type == MTS_WRITE) {
...
void pci_bridge_set_bus_info(struct pci_bridge *bridge, int pri_bus,
...
/* Add a PCI device */
struct pci_device *pci_dev_add(struct pci_bus *pci_bus, char *name,
                               u_int vendor_id, u_int product_id, int device,
                               int function, int irq, void *priv_data,
                               pci_init_t init, pci_reg_read_t read_register,
  ...
  if ((dev = pci_dev_lookup_local(pci_bus, device, function)) != NULL) {
    fprintf(stderr,
            "pci_dev_add: bus %s, device %d, function %d already "
            "registered (device '%s').\n",
...
struct pci_device *pci_dev_add_basic(struct pci_bus *pci_bus, char *name,
                                     u_int vendor_id, u_int product_id,
                                     int device, int function) {
  return (pci_dev_add(pci_bus, name, vendor_id, product_id, device, function,
...
int pci_dev_remove_by_id(struct pci_bus *pci_bus, int bus, int device,
                         int function) {
  struct pci_device *dev;
...
static void pci_bridge_write_reg(cpu_gen_t *cpu, struct pci_device *dev,
  ...
  case 0x18:
    ...
    vm_log(cpu->vm, "PCI",
           ...
           dev->pci_bus->bus, dev->device, dev->function, pri_bus, sec_bus,
           sub_bus);
...
static void pci_bus_show_dev_list(struct pci_bus *pci_bus) {
  ...
  for (dev = pci_bus->dev_list; dev; dev = dev->next) {
    printf("   %-18s: ID %4.4x:%4.4x, Bus %s, Dev. %2d, Func. %2d", dev->name,
           dev->vendor_id, dev->product_id, bus_id, dev->device, dev->function);
