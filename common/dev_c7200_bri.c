...
void *pa_4b_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
  ...
  if (offset >= MUNICH32_MEM_SIZE) {
    if (op_type == MTS_READ) {
      cpu_log(cpu, d->name,
              "read  access to offset = 0x%x, pc = 0x%llx "
              "(op_size=%u)\n",
  ...
  default:
    if (offset < MUNICH32_MEM_SIZE)
      return (m32_gen_access(&d->m32_data, cpu, offset - d->m32_offset, op_size,
                             op_type, data));
  }
 ...
 * pci_munich32_write()
 */
static void pci_munich32_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                               m_uint32_t value) {
  struct pa_4b_data *d = dev->priv_data;
...
int dev_c7200_pa_bri_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  pci_dev = pci_dev_add(card->pci_bus, card->dev_name, BRI_PCI_VENDOR_ID,
                        BRI_PCI_PRODUCT_ID, 0, 0, C7200_NETIO_IRQ, d, NULL,
                        pci_munich32_read, pci_munich32_write);
...
int dev_c7200_pa_bri_set_nio(vm_instance_t *vm, struct cisco_card *card,
  ...
  /* TEST */
  d->m32_data.tx_tid =
      ptask_add((ptask_callback)m32_tx_scan_all_channels, &d->m32_data, NULL);
