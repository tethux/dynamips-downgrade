...
int parse_board_id(m_uint8_t *buf, const char *id, int encode) {
  ...
  if (encode == 4) {
    ...
    res = sscanf(id, "%c%c%c%2hx%2hx%c%c%c%c", &buf[0], &buf[1], &buf[2],
                 (unsigned short int *)&buf[3], (unsigned short int *)&buf[4],
    ...
    res =
        sscanf(id, "%c%c%c%c%c%c%c%c%c%c%c", &buf[0], &buf[1], &buf[2], &buf[3],
...
int parse_mac_addr(n_eth_addr_t *addr, char *str) {
  ...
  res = sscanf(str, "%x:%x:%x:%x:%x:%x", &v[0], &v[1], &v[2], &v[3], &v[4],
               &v[5]);
...
char *n_eth_ntoa(char *buffer, n_eth_addr_t *addr, int format) {
  ...
  sprintf(buffer, str_format, addr->eth_addr_byte[0], addr->eth_addr_byte[1],
...
int ip_listen(char *ip_addr, int port, int sock_type, int max_fd,
              int fd_array[]) {
  struct addrinfo hints, *res, *res0;
  ...
  for (res = res0; (res && (nsock < max_fd)); res = res->ai_next) {
    ...
    if (setsockopt(fd_array[nsock], SOL_SOCKET, SO_REUSEADDR, &reuse,
                   sizeof(reuse)) != 0)
      perror(
          "Warning: ip_listen: setsockopt(SO_REUSEADDR): The same address-port "
          "combination can be retried after the TCP TIME_WAIT state expires.");
...
int pkt_ctx_analyze(n_pkt_ctx_t *ctx, m_uint8_t *pkt, size_t pkt_len) {
  ...
  case N_ETH_PROTO_IP: {
    ...
    if (((ip->ihl & 0xF0) != 0x40) ||
        ...
        ((len << 2) > ntohs(ip->tot_len)) || !ip_verify_cksum(ctx->ip))
      return (TRUE);
...
void pkt_ctx_dump(n_pkt_ctx_t *ctx) {
  printf("pkt=%p (len=%lu), flags=0x%8.8x, vlan_id=0x%4.4x, l3=%p, l4=%p\n",
         ctx->pkt, (u_long)ctx->pkt_len, ctx->flags, ctx->vlan_id, ctx->l3,
         ctx->l4);
}
