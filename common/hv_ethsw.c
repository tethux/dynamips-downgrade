...
static int cmd_create(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!(t = ethsw_create(argv[0]))) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to create Ethernet switch '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (registry_rename(argv[0], newname, OBJ_TYPE_ETHSW)) {
    ...
    ethsw_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename ETHSW '%s'", argv[0]);
    return (-1);
  }
  ...
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "ETHSW '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static int cmd_add_nio(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (ethsw_add_netio(t, argv[1]) == -1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_BINDING, 1,
                          "unable to bind NIO '%s' to switch '%s'", argv[1],
                          argv[0]);
    return (-1);
  }
 ...
 */
static int cmd_set_access_port(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  ethsw_table_t *t;
...
static int cmd_clear_mac_addr_table(hypervisor_conn_t *conn, int argc,
                                    char *argv[]) {
  ethsw_table_t *t;
...
static void cmd_show_mac_addr_entry(ethsw_table_t *t, ethsw_mac_entry_t *entry,
  ...
  hypervisor_send_reply(
      conn, HSC_INFO_MSG, 0, "%2.2x%2.2x.%2.2x%2.2x.%2.2x%2.2x  %u  %s",
      entry->mac_addr.eth_addr_byte[0], entry->mac_addr.eth_addr_byte[1],
      entry->mac_addr.eth_addr_byte[2], entry->mac_addr.eth_addr_byte[3],
      entry->mac_addr.eth_addr_byte[4], entry->mac_addr.eth_addr_byte[5],
      entry->vlan_id, entry->nio->name);
}

static int cmd_show_mac_addr_table(hypervisor_conn_t *conn, int argc,
                                   char *argv[]) {
  ethsw_table_t *t;
  ...
  ethsw_iterate_mac_addr_table(
      t, (ethsw_foreach_entry_t)cmd_show_mac_addr_entry, conn);
