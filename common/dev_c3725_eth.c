...
static int dev_c3725_mb_get_sub_info(vm_instance_t *vm, struct cisco_card *card,
                                     u_int port_id,
  ...
  *drv_array = dev_c3725_mb_wic_drivers;
 ...
 */
static int dev_c3725_nm_eth_init(vm_instance_t *vm, struct cisco_card *card,
  ...
  for (i = 0; i < data->nr_port; i++) {
    data->port[i] = dev_am79c971_init(
        vm, card->dev_name, interface_type, card->pci_bus,
        c3725_nm_get_pci_device(slot), c3725_net_irq_for_slot_port(slot, i));
  }
...
static int dev_c3725_nm_eth_shutdown(vm_instance_t *vm,
...
static int dev_c3725_nm_eth_set_nio(vm_instance_t *vm, struct cisco_card *card,
...
static int dev_c3725_nm_eth_unset_nio(vm_instance_t *vm,
 ...
 */
static int dev_c3725_nm_1fe_tx_init(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
static int dev_c3725_nm_16esw_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  /* Create the device */
  data = dev_nm_16esw_init(vm, card->dev_name, slot, card->pci_bus,
...
static int dev_c3725_nm_16esw_shutdown(vm_instance_t *vm,
                                       struct cisco_card *card) {
...
static int dev_c3725_nm_16esw_set_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id,
                                      netio_desc_t *nio) {
...
/* Show debug info */
static int dev_c3725_nm_16esw_show_info(vm_instance_t *vm,
                                        struct cisco_card *card) {
  struct nm_16esw_data *d = card->drv_info;
...
static int dev_c3725_gt96100_fe_init(vm_instance_t *vm,
                                     struct cisco_card *card) {
  if (card->slot_id != 0) {
    vm_error(vm, "dev_c3725_gt96100_fe_init: bad slot %u specified.\n",
             card->slot_id);
    return (-1);
  }

  /* Store device info into the router structure */
  card->drv_info = VM_C3725(vm)->gt_data;
  return (0);
}

/* Nothing to do, we never remove the system controller */
static int dev_c3725_gt96100_fe_shutdown(vm_instance_t *vm,
                                         struct cisco_card *card) {
  return (0);
}

/* Bind a Network IO descriptor */
static int dev_c3725_gt96100_fe_set_nio(vm_instance_t *vm,
                                        struct cisco_card *card, u_int port_id,
                                        netio_desc_t *nio) {
...
/* Unbind a Network IO descriptor */
static int dev_c3725_gt96100_fe_unset_nio(vm_instance_t *vm,
                                          struct cisco_card *card,
                                          u_int port_id) {
  struct gt_data *d = card->drv_info;
...
/* Show debug info */
static int dev_c3725_gt96100_show_info(vm_instance_t *vm,
                                       struct cisco_card *card) {
  struct gt_data *d = card->drv_info;
  dev_gt96100_show_info(d);
  return (0);
}
