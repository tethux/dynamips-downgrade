...
static forced_inline void ppc32_reload_cpu(u_char **ptr) {
  ppc_lwz(*ptr, ppc_r3, PPC_STACK_DECREMENTER + PPC_STACK_PARAM_OFFSET, ppc_r1);
}
...
static forced_inline void ppc32_cr_and(u_char **p, int rD, int rB) {
  ppc_and(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_andc(u_char **p, int rD, int rB) {
  ppc_andc(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_eqv(u_char **p, int rD, int rB) {
  ppc_eqv(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_nand(u_char **p, int rD, int rB) {
  ppc_nand(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_nor(u_char **p, int rD, int rB) {
  ppc_nor(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_or(u_char **p, int rD, int rB) {
  ppc_or(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_orc(u_char **p, int rD, int rB) {
  ppc_orc(*p, rD, rD, rB);
}
static forced_inline void ppc32_cr_xor(u_char **p, int rD, int rB) {
  ppc_xor(*p, rD, rD, rB);
}
...
static void ppc32_try_direct_far_jump(cpu_ppc_t *cpu, jit_op_t *iop,
  ...
  test1 = iop->ob_ptr;
  ppc_bc(iop->ob_ptr, PPC_BR_TRUE_UNLIKELY, ppc_crbf(ppc_cr0, PPC_BR_EQ), 0);

  /* Check block PC */
  ppc_lwz(iop->ob_ptr, ppc_r6, OFFSET(ppc32_jit_tcb_t, start_ia), ppc_r5);
  ppc_cmpw(iop->ob_ptr, ppc_cr0, ppc_r4, ppc_r6);
  test2 = iop->ob_ptr;
  ppc_bc(iop->ob_ptr, PPC_BR_FALSE_UNLIKELY, ppc_crbf(ppc_cr0, PPC_BR_EQ), 0);

  /* Jump to the code */
  ppc_lwz(iop->ob_ptr, ppc_r6, OFFSET(ppc32_jit_tcb_t, jit_insn_ptr), ppc_r5);
...
void ppc32_set_page_jump(cpu_ppc_t *cpu, ppc32_jit_tcb_t *b) {
  ...
  mono_ppc_flush_icache(b->jit_buffer->ptr, b->jit_ptr - b->jit_buffer->ptr);
}
...
_Unused static int ppc32_jit_get_tmp_hreg(cpu_ppc_t *cpu) { return (ppc_r12); }
...
/* BC - Branch Conditional */
DECLARE_INSN(BC) { return (ppc32_emit_bc_generic(cpu, b, insn, "bc")); }

/* BC - Branch Conditional (Condition Check only) */
DECLARE_INSN(BCC) { return (ppc32_emit_bc_generic(cpu, b, insn, "bcc")); }
...
DECLARE_INSN(CRAND) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crand", ppc32_cr_and));
}
/* CRANDC - Condition Register AND with Complement */
DECLARE_INSN(CRANDC) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crandc", ppc32_cr_andc));
}
/* CREQV - Condition Register EQV */
DECLARE_INSN(CREQV) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "creqv", ppc32_cr_eqv));
}
/* CRNAND - Condition Register NAND */
DECLARE_INSN(CRNAND) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crnand", ppc32_cr_nand));
}
/* CRNOR - Condition Register NOR */
DECLARE_INSN(CRNOR) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crnor", ppc32_cr_nor));
}
/* CROR - Condition Register OR */
DECLARE_INSN(CROR) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "cror", ppc32_cr_or));
}
/* CRORC - Condition Register OR with Complement */
DECLARE_INSN(CRORC) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crorc", ppc32_cr_orc));
}
/* CRXOR - Condition Register XOR */
DECLARE_INSN(CRXOR) {
  return (ppc32_emit_cr_logical(cpu, b, insn, "crxor", ppc32_cr_xor));
}
...
DECLARE_INSN(MFCR) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    ppc_rlwimi(iop->ob_ptr, ppc_r4, ppc_r5, 28 - (i << 2), i << 2,
               (i << 2) + 3);
  }
  ppc32_store_gpr(&iop->ob_ptr, rt, ppc_r4);
  return (0);
