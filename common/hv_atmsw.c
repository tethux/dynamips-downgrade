...
static int cmd_create(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!(t = atmsw_create_table(argv[0]))) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to create ATM switch '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (registry_rename(argv[0], newname, OBJ_TYPE_ATMSW)) {
    ...
    atmsw_release(argv[0]);
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename ATMSW '%s'", argv[0]);
    return (-1);
  }
  ...
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "ATMSW '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static int cmd_create_vpc(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (atmsw_create_vpc(t, argv[1], atoi(argv[2]), argv[3], atoi(argv[4])) ==
      -1) {
    atmsw_release(argv[0]);
...
static int cmd_delete_vpc(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (atmsw_delete_vpc(t, argv[1], atoi(argv[2]), argv[3], atoi(argv[4])) ==
      -1) {
    atmsw_release(argv[0]);
...
static int cmd_create_vcc(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  /* create the connection */
  if (atmsw_create_vcc(t, argv[1], atoi(argv[2]), atoi(argv[3]), argv[4],
...
static int cmd_delete_vcc(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  /* create the connection */
  if (atmsw_delete_vcc(t, argv[1], atoi(argv[2]), atoi(argv[3]), argv[4],
                       atoi(argv[5]), atoi(argv[6])) == -1) {
    atmsw_release(argv[0]);
