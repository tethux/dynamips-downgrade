...
static m_uint16_t eeprom_msfc1_data[128] = {
    0xabab, 0x0190, 0x1262, 0x0100, 0x0002, 0x6003, 0x00cf, 0x4369, 0x7363,
    0x6f20, 0x5379, 0x7374, 0x656d, 0x732c, 0x2049, 0x6e63, 0x2e00, 0x5753,
    0x2d46, 0x3630, 0x3031, 0x2d52, 0x5346, 0x4300, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x3733, 0x2d37, 0x3135, 0x302d, 0x3036, 0x0000, 0x0000, 0x0000,
    0x4130, 0x3100, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x012d, 0x0000, 0x0000, 0x0009, 0x0005,
    0x0001, 0x0003, 0x0001, 0x0001, 0x0002, 0x00cf, 0xffbf, 0x0000, 0x0000,
    0x6003, 0x0162, 0x0afd, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0005, 0x00e0, 0xaabb, 0xcc00, 0x0100, 0x0100, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x1401, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x1000, 0x4b3c, 0x4132, 0x8080, 0x8080,
    0x8080, 0x8080, 0x8080, 0x8080, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff,
    0xffff, 0xffff,
};
...
/* Remove EOBC */
static int dev_c6msfc1_eobc_shutdown(vm_instance_t *vm,
...
static int dev_c6msfc1_eobc_unset_nio(vm_instance_t *vm,
                                      struct cisco_card *card, u_int port_id) {
  struct dec21140_data *d = card->drv_info;
...
/* Remove EOBC */
static int dev_c6msfc1_ibc_shutdown(vm_instance_t *vm,
                                    struct cisco_card *card) {
...
/* Unbind a Network IO descriptor */
static int dev_c6msfc1_ibc_unset_nio(vm_instance_t *vm, struct cisco_card *card,
                                     u_int port_id) {
  struct i8254x_data *d = card->drv_info;
...
static int c6msfc1_nvram_extract_config(vm_instance_t *vm,
                                        u_char **startup_config,
                                        size_t *startup_len,
                                        u_char **private_config,
                                        size_t *private_len) {
  int ret;

  ret = generic_nvram_extract_config(
      vm, "nvram", vm->nvram_rom_space, 0,
      C6MSFC1_NVRAM_ADDR + vm->nvram_rom_space, FS_NVRAM_FORMAT_ABSOLUTE_C6,
      startup_config, startup_len, private_config, private_len);

  return (ret);
}

/* Directly push the IOS configuration to the NVRAM device */
static int c6msfc1_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                     size_t startup_len, u_char *private_config,
                                     size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(
      vm, "nvram", vm->nvram_size * 1024, vm->nvram_rom_space, 0,
      C6MSFC1_NVRAM_ADDR + vm->nvram_rom_space, FS_NVRAM_FORMAT_ABSOLUTE_C6,
      startup_config, startup_len, private_config, private_len);
...
static inline void c6msfc1_net_irq_get_slot_port(u_int irq, u_int *slot,
                                                 u_int *port) {
...
_Unused static int c6msfc1_burn_mac_addr(c6msfc1_t *router,
                                         n_eth_addr_t *addr) {
  ...
  if (eeprom_ver != 1) {
    vm_error(router->vm,
             "c6msfc1_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static int c6msfc1_init_platform(c6msfc1_t *router) {
  ...
  /* NVRAM and calendar */
  dev_nvram_init(vm, "nvram", C6MSFC1_NVRAM_ADDR, vm->nvram_size * 1024,
                 &vm->conf_reg);
  ...
  if (!vm->rom_filename) {
    ...
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C6MSFC1_ROM_ADDR,
                 vm->rom_size * 1048576);
  }

  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C6MSFC1_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
...
static int c6msfc1_init_instance(vm_instance_t *vm) {
  ...
  if ((vm->rom_filename != NULL) &&
      (mips64_load_elf_image(cpu0, vm->rom_filename, 0, &rom_entry_point) <
       0)) {
    vm_error(vm,
             "unable to load alternate ROM '%s', "
...
/* Get MAC address MSB */
static u_int c6msfc1_get_mac_addr_msb(void) { return (0xC6); }
