...
struct nvram_format {
  ...
  u_int format;
    ...
    {"c2600", "C2600", C2600_NVRAM_ROM_RES_SIZE * 4, 0, 0,
     FS_NVRAM_FORMAT_SCALE_4},
    {"c3600", "3600", C3600_NVRAM_ROM_RES_SIZE, 0, 0, FS_NVRAM_FORMAT_DEFAULT},
    {"c7200", "7200", C7200_NVRAM_ROM_RES_SIZE, 0,
     C7200_NVRAM_ADDR + C7200_NVRAM_ROM_RES_SIZE, FS_NVRAM_FORMAT_ABSOLUTE},
    {"c7200-npe-g2", "7200", C7200_NVRAM_ROM_RES_SIZE, 0,
     C7200_G2_NVRAM_ADDR + C7200_NVRAM_ROM_RES_SIZE, FS_NVRAM_FORMAT_ABSOLUTE},
    {"c6msfc1", NULL, C6MSFC1_NVRAM_ROM_RES_SIZE, 0,
     C6MSFC1_NVRAM_ADDR + C6MSFC1_NVRAM_ROM_RES_SIZE,
     FS_NVRAM_FORMAT_ABSOLUTE_C6},
    {"c2691", NULL, C2691_NVRAM_OFFSET, C2691_NVRAM_SIZE, 0,
     FS_NVRAM_FORMAT_WITH_BACKUP},
    {"c3725", NULL, C3725_NVRAM_OFFSET, C3725_NVRAM_SIZE, 0,
     FS_NVRAM_FORMAT_WITH_BACKUP}, // XXX same as c2691
    {"c3745", NULL, C3745_NVRAM_OFFSET, C3745_NVRAM_SIZE, 0,
     FS_NVRAM_FORMAT_WITH_BACKUP},
    {"c7200-npe-g1", NULL, C7200_NVRAM_ROM_RES_SIZE, 0,
     C7200_G1_NVRAM_ADDR + C7200_NVRAM_ROM_RES_SIZE,
     FS_NVRAM_FORMAT_ABSOLUTE}, // XXX didn't find working image
    {NULL, NULL, 0, 0, 0, 0}};
...
int nvram_export_config(const char *nvram_filename,
  ...
  for (fmt = &nvram_formats[0];; fmt++) {
    ...
    if (fmt->rom_res_0x200) {
      ...
      if (data_len < 0x200 + len ||
          memcmp(data + 0x200, fmt->rom_res_0x200, len)) {
        continue; // must match
      }
    }
    ...
    if (fs_nvram_verify(fs, FS_NVRAM_VERIFY_ALL) ||
        fs_nvram_read_config(fs, &startup_config, &startup_len, &private_config,
                             &private_len)) {
      fs_nvram_close(fs);
      fs = NULL;
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3 || argc > 4) {
    fprintf(stderr, "Usage: %s nvram_file config_file [private_file]\n",
            argv[0]);
    fprintf(stderr, "\n");
    fprintf(stderr, "This tools extracts 'startup-config' and 'private-config' "
                    "from NVRAM.\n");
    fprintf(stderr, "  nvram_file   - file that contains the NVRAM data\n");
    fprintf(stderr, "                 (on some platforms, NVRAM is simulated "
                    "inside the ROM)\n");
