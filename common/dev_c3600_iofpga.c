...
static struct net_irq_distrib net_irq_dist[C3600_MAX_NM_BAYS] = {
    {16, 1, 0},  {24, 0, 24}, {0, 0, 16}, {8, 0, 28},
    {32, 0, 20}, {32, 0, 8},  {32, 0, 0},
};
...
void dev_c3600_iofpga_net_set_irq(struct c3600_iofpga_data *d, u_int slot,
                                  u_int port) {
...
void dev_c3600_iofpga_net_clear_irq(struct c3600_iofpga_data *d, u_int slot,
                                    u_int port) {
  struct net_irq_distrib *irq_dist;
 ...
 */
static void *dev_c3620_c3640_iofpga_access(cpu_gen_t *cpu, struct vdevice *dev,
 ...
 */
static void *dev_c3660_iofpga_access(cpu_gen_t *cpu, struct vdevice *dev,
                                     m_uint32_t offset, u_int op_size,
                                     u_int op_type, m_uint64_t *data) {
  struct c3600_iofpga_data *d = dev->priv_data;
  u_int slot;
  ...
  case 0x1000f:
    ...
    if (op_type == MTS_WRITE) {
      nmc93cX6_write(&d->router->c3660_nm_eeprom_group[slot], (u_int)(*data));
    } else {
  ...
  case 0x20006:
    if (op_type == MTS_WRITE) {
      nmc93cX6_write(&d->router->c3660_nm_eeprom_group[0], (u_int)(*data));
    } else {
...
static void dev_c3600_iofpga_shutdown(vm_instance_t *vm,
...
int dev_c3600_iofpga_init(c3600_t *router, m_uint64_t paddr, m_uint32_t len) {
  ...
  default:
    fprintf(stderr, "C3600 '%s': invalid chassis ID %d\n", router->vm->name,
            router->chassis_driver->chassis_id);
    free(d);
    return (-1);
  }
