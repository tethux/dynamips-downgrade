...
void vtty_set_ctrlhandler(int n) { ctrl_code_ok = n; }
/* Allow the user to disable the telnet message for AUX and CONSOLE */
void vtty_set_telnetmsg(int n) { telnet_message_ok = n; }
...
static int vtty_tcp_conn_wait(vtty_t *vtty) {
  ...
  /* Try to use the console binding address first, then fallback to the global
   * binding address */
  addr = (console_binding_addr && strlen(console_binding_addr))
  ...
  for (res = res0; (res && (nsock < VTTY_MAX_FD)); res = res->ai_next) {
    ...
    if (setsockopt(vtty->fd_array[nsock], SOL_SOCKET, SO_REUSEADDR, &one,
                   sizeof(one)) < 0)
      perror("vtty_tcp_waitcon: setsockopt(SO_REUSEADDR)");

    if (setsockopt(vtty->fd_array[nsock], SOL_SOCKET, SO_KEEPALIVE, &one,
                   sizeof(one)) < 0)
      perror("vtty_tcp_waitcon: setsockopt(SO_KEEPALIVE)");

    // Send telnet packets asap. Dont wait to fill packets up
    if (setsockopt(vtty->fd_array[nsock], SOL_TCP, TCP_NODELAY, &one,
                   sizeof(one)) < 0)
    ...
    vm_log(vtty->vm, "VTTY",
           "%s: waiting connection on tcp port %d for protocol %s (FD %d)\n",
...
static int vtty_tcp_conn_wait(vtty_t *vtty) {
  ...
  if (setsockopt(vtty->fd_array[0], SOL_SOCKET, SO_REUSEADDR, &one,
                 sizeof(one)) < 0) {
    perror("vtty_tcp_waitcon: setsockopt(SO_REUSEADDR)");
    goto error;
  }

  if (setsockopt(vtty->fd_array[0], SOL_SOCKET, SO_KEEPALIVE, &one,
                 sizeof(one)) < 0) {
    perror("vtty_tcp_waitcon: setsockopt(SO_KEEPALIVE)");
    goto error;
  }
  ...
  // Send telnet packets asap. Dont wait to fill packets up
  if (setsockopt(vtty->fd_array[0], SOL_TCP, TCP_NODELAY, &one, sizeof(one)) <
      0) {
    perror("vtty_tcp_waitcon: setsockopt(TCP_NODELAY)");
    goto error;
  }
  ...
  *fd_slot = fd;
  ...
  if (telnet_message_ok == 1) {
    ...
    if (vtty->vm->status != VM_STATUS_RUNNING)
      fd_printf(fd, 0,
                "\r\n!!! WARNING - VM is not running, will be unresponsive "
                "(status=%d) !!!\r\n",
                vtty->vm->status);
    vtty_flush(vtty);
  }
...
static int remote_control_mips64(vtty_t *vtty, char c, cpu_mips_t *cpu) {
  ...
  case 'b':
    printf("\nCPU0: %u JIT compiled pages [Exec Area Pages: %lu/%lu]\n",
           cpu->compiled_pages, (u_long)cpu->exec_page_alloc,
...
static void vtty_read_and_store(vtty_t *vtty, int *fd_slot) {
  ...
  case VTTY_INPUT_TELNET_SB2:
    vtty->telnet_qual = c;
    if ((vtty->telnet_opt == TELOPT_TTYPE) && (vtty->telnet_qual == TELQUAL_IS))
