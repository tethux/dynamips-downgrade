...
static m_uint8_t cis_table[] = {
    ...
    0x2b,
    0x04,
    0x01, /* PCMCIA 2.0/2.1 / JEIDA 4.1/4.2 */
    0x44,
    0x79,
    0x6e,
    0x61,
    ...
    0x5f, /* P0(Sleep) | P1(Standy) | P2(Idle) | P3(Auto) | N(3F7/377 Register
             Inhibit Available) | I(IOIS16# on Twin Card) */
    /* CISTPL_CONFIG */
    0x1a,
    0x05,
    ...
    0x03, /* TPCC_LAST=3 */
    0x00,
    0x02, /* TPCC_RADR(Configuration Register Base Address)=0x0200 */
    ...
    0xa1, /* VCC power-description-structure only | Single 2-byte length
             specified | Misc */
    ...
    0x75, /* Peak I=80mA */
    0x08,
    0x00, /* Card Address=0 | Host Address=0x0008 * 256 bytes */
    ...
    0x5d, /* Max V=5V */
    0x75, /* Peak I=80mA */
    0x64, /* IOAddrLines=4 | All registers are accessible by both 8-bit or
             16-bit accesses */
    0xf0,
    0xff,
    0xff, /* Mask | Level | Pulse | Share | IRQ0..IRQ15 */
    0x21, /* Max Twin Cards=1 | Power Down */
    /* CISTPL_CFTABLE_ENTRY */
    0x1b,
    0x06,
    ...
    0x5d, /* Max V=5V */
    0x75, /* Peak I=80mA */
    0xea, /* IOAddrLines=10 | All registers are accessible by both 8-bit or
             16-bit accesses | Range */
    0x61, /* Number of I/O Address Ranges=2 | Size of Address=2 | Size of
             Length=1 */
    0xf0,
    0x01,
    0x07, /* Address=0x1F0, Length=8 */
    0xf6,
    0x03,
    0x01, /* Address=0x3F6, Length=2 */
    ...
    0x70,
    0x01,
    0x07, /* Address=0x170, Length=8 */
    0x76,
    0x03,
    0x01, /* Address=0x376, Length=2 */
    0xee, /* IRQ14 | Level | Pulse | Share */
    0x21, /* Max Twin Cards=1 | Power Down */
    /* CISTPL_CFTABLE_ENTRY */
    0x1b,
...
static inline m_uint32_t chs_to_lba(struct pcmcia_disk_data *d, u_int cyl,
                                    u_int head, u_int sect) {
...
static int disk_format(struct pcmcia_disk_data *d) {
  ...
  part->nr_sectors =
      d->nr_heads * d->nr_cylinders * d->sects_per_track - part->lba;
...
static int disk_create(struct pcmcia_disk_data *d) {
  ...
  disk_len =
      (off_t)d->nr_heads * d->nr_cylinders * d->sects_per_track * SECTOR_SIZE;
...
#if DEBUG_ACCESS
  if (op_type == MTS_READ) {
    cpu_log(cpu, d->dev.name, "reading offset 0x%5.5x at pc=0x%llx (size=%u)\n",
