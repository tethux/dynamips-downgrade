...
int netio_release(char *name) { return (registry_unref(name, OBJ_TYPE_NIO)); }
...
static void netio_reg_save_config(registry_entry_t *entry, void *opt,
                                  int *err) {
...
static int netio_unix_create_socket(netio_unix_desc_t *nud) {
  ...
  strncpy(local_sock.sun_path, nud->local_filename,
          sizeof(local_sock.sun_path) - 1);
...
static ssize_t netio_unix_send(netio_unix_desc_t *nud, void *pkt,
                               size_t pkt_len) {
  return (sendto(nud->fd, pkt, pkt_len, 0, (struct sockaddr *)&nud->remote_sock,
...
static int netio_vde_create(netio_vde_desc_t *nvd, char *control, char *local) {
  ...
  res = connect(nvd->ctrl_fd, (struct sockaddr *)&ctrl_sock, sizeof(ctrl_sock));
...
static void netio_vde_save_cfg(netio_desc_t *nio, FILE *fd) {
  ...
  fprintf(fd, "nio create_vde %s %s %s\n", nio->name, nvd->remote_sock.sun_path,
          nvd->local_filename);
}
...
int netio_rxl_add(netio_desc_t *nio, netio_rx_handler_t rx_handler, void *arg1,
                  void *arg2) {
  struct netio_rx_listener *rxl;
