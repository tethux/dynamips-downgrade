...
static int mpc860_idma_fetch_bd(struct mpc860_data *d, m_uint16_t bd_addr,
                                struct mpc860_idma_bd *bd) {
  if ((bd_addr < MPC860_DPRAM_OFFSET) ||
      (bd_addr >= MPC860_DPRAM_END - MPC860_IDMA_BD_SIZE))
    return (-1);
...
#if DEBUG_IDMA
  MPC_LOG(d,
          "fetched IDMA BD at 0x%4.4x, src_bp=0x%8.8x, dst_bp=0x%8.8x "
          "len=%d\n",
...
static int mpc860_spi_fetch_bd(struct mpc860_data *d, m_uint16_t bd_addr,
  ...
  if ((bd_addr < MPC860_DPRAM_OFFSET) ||
      (bd_addr >= MPC860_DPRAM_END - MPC860_SPI_BD_SIZE))
    return (-1);
...
void mpc860_spi_set_tx_callback(struct mpc860_data *d,
                                mpc860_spi_tx_callback_t cbk, void *user_arg) {
...
static void mpc860_scc_init_rx_tx_params(struct mpc860_data *d,
                                         u_int scc_chan) {
...
static int mpc860_scc_fetch_bd(struct mpc860_data *d, m_uint16_t bd_addr,
  ...
  if ((bd_addr < MPC860_DPRAM_OFFSET) ||
      (bd_addr >= MPC860_DPRAM_END - MPC860_SCC_BD_SIZE))
    return (-1);
...
static int mpc860_scc_handle_rx_pkt(netio_desc_t *nio, u_char *pkt,
                                    ssize_t pkt_len, struct mpc860_data *d,
                                    void *arg) {
...
int mpc860_scc_set_nio(struct mpc860_data *d, u_int scc_chan,
                       netio_desc_t *nio) {
  struct mpc860_scc_chan *chan;
  ...
  chan->nio = nio;
  netio_rxl_add(nio, (netio_rx_handler_t)mpc860_scc_handle_rx_pkt, d,
                (void *)(u_long)scc_chan);
  return (0);
}
...
static int mpc860_fec_handle_rx_pkt(netio_desc_t *nio, u_char *pkt,
                                    ssize_t pkt_len, struct mpc860_data *d,
                                    void *arg) {
  ...
  if (mpc860_fec_link_is_up(d)) {
    ...
    if (!(regs[LX970A_CR] & LX970A_CR_ANENABLE)) {
      ...
      if ((regs[LX970A_ANAR] & LX970A_ANAR_100TX_FD)) {
        ...
        /* 10Mbps full duplex */
        regs[LX970A_CSR] =
            (LX970A_CSR_DUPLEXMODE | (regs[LX970A_CSR] & ~LX970A_CSR_SPEED));
      } else {
        /* 10Mbps half duplex */
    ...
    d->fec_mii_regs[LX970A_SR] &=
        ~(LX970A_SR_ANCOMPLETE | LX970A_SR_LINKSTATUS);
    d->fec_mii_regs[LX970A_CSR] &= ~(LX970A_CSR_LINK | LX970A_CSR_DUPLEXMODE |
                                     LX970A_CSR_SPEED | LX970A_CSR_ANCOMPLETE);
  }
}
...
static void mpc860_fec_mii_read_access(struct mpc860_data *d, u_int phy,
                                       u_int reg) {
  m_uint16_t res;
...
/* MII register read access */
static void mpc860_fec_mii_write_access(struct mpc860_data *d, u_int phy,
                                        u_int reg) {
...
#if DEBUG_FEC
  MPC_LOG(
      d, "FEC: Writing 0x%4.4x to MII phy %d reg %d at ia=0x%4.4x,lr=0x%4.4x\n",
      data, phy, reg, CPU_PPC32(d->vm->boot_cpu)->ia,
      CPU_PPC32(d->vm->boot_cpu)->lr);
#endif
...
#if DEBUG_ACCESS
  if (op_type == MTS_READ) {
    cpu_log(cpu, d->name, "read from offset 0x%x, pc=0x%llx (size=%u)\n",
            offset, cpu_get_pc(cpu), op_size);
  } else {
    cpu_log(cpu, d->name,
            "write to offset 0x%x, value=0x%llx, pc=0x%llx (size=%u)\n", offset,
            *data, cpu_get_pc(cpu), op_size);
  }
#endif
...
int dev_mpc860_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
                    m_uint32_t len) {
