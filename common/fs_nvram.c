...
static inline void be_to_native32(m_uint32_t *val) {
  ...
  u.val = *val;
  *val = (((m_uint32_t)u.b[0] << 24) | ((m_uint32_t)u.b[1] << 16) |
          ((m_uint32_t)u.b[2] << 8) | u.b[3]);
}

/** Convert startup-config header values from big endian to native. */
static void
be_to_native_header_startup(struct fs_nvram_header_startup_config *head) {
...
static void
be_to_native_header_private(struct fs_nvram_header_private_config *head) {
  be_to_native16(&head->magic);
  be_to_native16(&head->format);
  be_to_native32(&head->start);
  be_to_native32(&head->end);
...
static void
native_to_be_header_startup(struct fs_nvram_header_startup_config *head) {
...
static void
native_to_be_header_private(struct fs_nvram_header_private_config *head) {
  native_to_be16(&head->magic);
  native_to_be16(&head->format);
  native_to_be32(&head->start);
  native_to_be32(&head->end);
 ...
 */
static int uncompress_LZC(const u_char *in_data, u_int in_len, u_char *out_data,
                          u_int out_len) {
...
#define LZC_NUM_BITS_MAX 16
  ...
  if (in_len < 3 || (in_data == NULL && in_len > 0) ||
  ...
  parents = (m_uint16_t *)malloc(numItems * sizeof(*parents));
  if (parents == NULL)
    goto err_memory;
  suffixes = (m_uint8_t *)malloc(numItems * sizeof(*suffixes));
  if (suffixes == NULL)
    goto err_memory;
  stack = (m_uint8_t *)malloc(numItems * sizeof(*stack));
  if (stack == NULL)
    goto err_memory;
  ...
  for (;;) {
    ...
    m_uint32_t symbol = buf[bytePos] | ((m_uint32_t)buf[bytePos + 1] << 8) |
                        ((m_uint32_t)buf[bytePos + 2] << 16);
    symbol >>= (bitPos & 7);
    symbol &= (1 << numBits) - 1;
    bitPos += numBits;
...
static inline m_uint32_t fs_nvram_address_of(fs_nvram_t *fs,
                                             m_uint32_t offset) {
...
static inline m_uint32_t fs_nvram_offset_of(fs_nvram_t *fs,
                                            m_uint32_t address) {
  if ((fs->flags & FS_NVRAM_FLAG_ADDR_ABSOLUTE))
...
_Unused static void fs_nvram_write32(fs_nvram_t *fs, u_int offset,
                                     m_uint32_t val) {
...
static void fs_nvram_memcpy_from(fs_nvram_t *fs, u_int offset, u_char *data,
                                 u_int len) {
  u_int i;
...
static void fs_nvram_memcpy_to(fs_nvram_t *fs, u_int offset, const u_char *data,
                               u_int len) {
  u_int i;
...
static void fs_nvram_update_checksum(fs_nvram_t *fs) {
  ...
  fs_nvram_write16(
      fs,
      sizeof(struct fs_nvram_header) +
          offsetof(struct fs_nvram_header_startup_config, checksum),
      0x0000);
...
static inline u_char *fs_nvram_read_data(fs_nvram_t *fs, u_int offset,
                                         u_int len) {
  u_char *data;
...
static void fs_nvram_create(fs_nvram_t *fs) {
  ...
  fs_nvram_write16(fs, offsetof(struct fs_nvram_header, magic),
                   FS_NVRAM_MAGIC_FILESYSTEM);
  fs_nvram_write16(
      fs,
      sizeof(struct fs_nvram_header) +
          offsetof(struct fs_nvram_header_startup_config, checksum),
...
static m_uint8_t fs_nvram_read_byte_with_backup(fs_nvram_t *fs, u_int offset) {
  ...
  {
    m_uint8_t *ptr2 =
        (m_uint8_t *)fs->base + fs_nvram_offset2_with_backup(fs, offset);
    if (*ptr1 != *ptr2)
      fprintf(stderr,
              "fs_nvram_read_byte_with_backup: data in backup filesystem "
              "doesn't match (offset=%u, offset1=%u, offset2=%u, "
              "normal=0x%02X, backup=0x%02X)\n",
              offset, fs_nvram_offset1_with_backup(fs, offset),
              fs_nvram_offset2_with_backup(fs, offset), *ptr1, *ptr2);
  }
#endif
...
static void fs_nvram_write_byte_with_backup(fs_nvram_t *fs, u_int offset,
                                            m_uint8_t val) {
  m_uint8_t *ptr1;
  m_uint8_t *ptr2;
...
fs_nvram_t *fs_nvram_open(u_char *base, size_t len, m_uint32_t addr,
                          u_int flags) {
  struct fs_nvram *fs;
  size_t len_div = 1;
  ...
  if (base == NULL || len < sizeof(struct fs_nvram_header) * len_div ||
      len % (FS_NVRAM_SECTOR_SIZE * len_div) != 0) {
    errno = EINVAL;
  ...
  fs->backup = (flags & FS_NVRAM_FLAG_WITH_BACKUP)
                   ? m_min(fs->len, FS_NVRAM_NORMAL_FILESYSTEM_BLOCK1)
                   : 0;
  fs->read_byte = (flags & FS_NVRAM_FLAG_WITH_BACKUP)
                      ? fs_nvram_read_byte_with_backup
                      : fs_nvram_read_byte;
  fs->write_byte = (flags & FS_NVRAM_FLAG_WITH_BACKUP)
                       ? fs_nvram_write_byte_with_backup
                       : fs_nvram_write_byte;

  if (FS_NVRAM_MAGIC_FILESYSTEM !=
      fs_nvram_read16(fs, offsetof(struct fs_nvram_header, magic))) {
 ...
 */
int fs_nvram_read_config(fs_nvram_t *fs, u_char **startup_config,
                         size_t *startup_len, u_char **private_config,
                         size_t *private_len) {
  int err;
  size_t off;
  u_char *buf;
  ...
  if (FS_NVRAM_FORMAT_RAW == startup_head.format) {
    ...
    if (startup_config) {
      ...
      *startup_config = (u_char *)malloc(startup_head.uncompressed_len + 1);
      ...
      err = uncompress_LZC(buf, startup_head.len, *startup_config,
                           startup_head.uncompressed_len);
      if (err)
        goto err_uncompress;
 ...
 * Returns 0 on success.
 */
int fs_nvram_write_config(fs_nvram_t *fs, const u_char *startup_config,
                          size_t startup_len, const u_char *private_config,
                          size_t private_len) {
  size_t len;
  size_t off;
  size_t padding;
  ...
  if (fs == NULL || (startup_config == NULL && startup_len > 0) ||
      (private_config == NULL && private_len > 0))
    return (EINVAL); // invalid argument

  // check space and padding
  // XXX ignores normal files in NVRAM
  len = sizeof(struct fs_nvram_header) +
        sizeof(struct fs_nvram_header_startup_config) + startup_len;
  ...
  // old length
  len = sizeof(struct fs_nvram_header);
  if (FS_NVRAM_MAGIC_STARTUP_CONFIG ==
      fs_nvram_read16(fs, len + offsetof(typeof(startup_head), magic))) {
    len += fs_nvram_read32(fs, len + offsetof(typeof(startup_head), len));
    if (len % 4 != 0)
      len += 8 - len % 4;

    if (FS_NVRAM_MAGIC_PRIVATE_CONFIG ==
        fs_nvram_read16(fs, len + offsetof(typeof(private_head), magic)))
  ...
  startup_head.unk1 = (fs->flags & FS_NVRAM_FLAGS_UNK1_0C01)   ? 0x0C01
                      : (fs->flags & FS_NVRAM_FLAGS_UNK1_0C03) ? 0x0C03
                                                               : 0x0C04;
  startup_head.start = fs_nvram_address_of(
      fs, sizeof(struct fs_nvram_header) +
              sizeof(struct fs_nvram_header_startup_config));
  ...
  private_head.start = startup_head.end + padding +
                       sizeof(struct fs_nvram_header_private_config);
  ...
  fs_nvram_memcpy_to(fs, off, (const u_char *)&startup_head,
                     sizeof(struct fs_nvram_header_startup_config));
  ...
  fs_nvram_memcpy_to(fs, off, (const u_char *)&private_head,
                     sizeof(struct fs_nvram_header_private_config));
...
int fs_nvram_verify(fs_nvram_t *fs, u_int what) {
  ...
  if ((what & FS_NVRAM_VERIFY_BACKUP)) {
    if ((fs->flags & FS_NVRAM_FLAG_WITH_BACKUP)) {
      for (offset = 0; offset < fs->len; offset++) {
        ...
        if (b1 != b2)
          return (FS_NVRAM_ERR_BACKUP_MISSMATCH); // data is corrupted? length
                                                  // is wrong?
      }
    }
  }
  ...
  if ((what & FS_NVRAM_VERIFY_CONFIG)) {
    ...
    offset = sizeof(struct fs_nvram_header);
    fs_nvram_memcpy_from(fs, offset, (u_char *)&startup_head,
                         sizeof(startup_head));
    be_to_native_header_startup(&startup_head);
    if (FS_NVRAM_MAGIC_STARTUP_CONFIG == startup_head.magic) {
      if (startup_head.end != startup_head.start + startup_head.len ||
          startup_head.len > fs->len)
        return (FS_NVRAM_ERR_INVALID_ADDRESS); // data is corrupted?
      if (startup_head.start < fs->addr ||
          startup_head.end > fs->addr + fs->len)
      ...
      fs_nvram_memcpy_from(fs, offset, (u_char *)&private_head,
                           sizeof(private_head));
      be_to_native_header_private(&private_head);
      if (FS_NVRAM_MAGIC_PRIVATE_CONFIG == private_head.magic) {
        if (private_head.end != private_head.start + private_head.len ||
            private_head.len > fs->len)
          return (FS_NVRAM_ERR_INVALID_ADDRESS); // data is corrupted?
        if (private_head.start < fs->addr ||
            private_head.end > fs->addr + fs->len)
          return (
              FS_NVRAM_ERR_INVALID_ADDRESS); // fs->addr has the wrong value?
        if (private_head.end != private_head.start + private_head.len)
          return (FS_NVRAM_ERR_INVALID_ADDRESS); // data is corrupted?
      }
    }
