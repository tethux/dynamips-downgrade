...
static int c3745_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(
      vm, "rom", C3745_NVRAM_OFFSET, C3745_NVRAM_SIZE, 0,
      FS_NVRAM_FORMAT_WITH_BACKUP, startup_config, startup_len, private_config,
      private_len);
...
static int c3745_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                   size_t startup_len, u_char *private_config,
                                   size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(vm, "rom", vm->rom_size * 1048576,
                                  C3745_NVRAM_OFFSET, C3745_NVRAM_SIZE, 0,
                                  FS_NVRAM_FORMAT_WITH_BACKUP, startup_config,
...
static inline void c3745_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
static int c3745_burn_mac_addr(c3745_t *router, n_eth_addr_t *addr) {
  ...
  switch (eeprom_ver) {
  case 0:
    cisco_eeprom_set_region(&router->sys_eeprom[2], 2, addr->eth_addr_byte, 6);
    break;

  case 4:
    if (!cisco_eeprom_v4_find_field(&router->sys_eeprom[2], 0xC3, &offset)) {
  ...
  default:
    vm_error(router->vm,
             "c3745_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static int c3745_init_gt96100(c3745_t *router) {
  ...
  if (dev_gt96100_init(vm, "gt96100", C3745_GT96K_ADDR, 0x200000,
                       C3745_GT96K_IRQ, C3745_EXT_IRQ,
                       c3745_net_irq_for_slot_port(0, 0), 255) == -1)
    return (-1);
...
static int c3745_init_platform(c3745_t *router) {
  ...
  if (!(obj =
            dev_flash_init(vm, "rom", C3745_ROM_ADDR, vm->rom_size * 1048576)))
    goto err;
  ...
  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C3745_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
...
/* Get MAC address MSB */
static u_int c3745_get_mac_addr_msb(void) { return (0xC4); }
...
int c3745_platform_register(void) {
  return (vm_platform_register(&c3745_platform));
}
