...
static int c3725_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(
      vm, "rom", C3725_NVRAM_OFFSET, C3725_NVRAM_SIZE, 0,
...
static int c3725_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                   size_t startup_len, u_char *private_config,
                                   size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(vm, "rom", vm->rom_size * 1048576,
                                  C3725_NVRAM_OFFSET, C3725_NVRAM_SIZE, 0,
...
static inline void c3725_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
static int c3725_burn_mac_addr(c3725_t *router, n_eth_addr_t *addr) {
  ...
  case 4:
    if (!cisco_eeprom_v4_find_field(&router->mb_eeprom, 0xC3, &offset)) {
      cisco_eeprom_set_region(&router->mb_eeprom, offset, addr->eth_addr_byte,
                              6);
    }
    break;
  ...
  default:
    vm_error(router->vm,
             "c3725_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static int c3725_init_gt96100(c3725_t *router) {
  ...
  if (dev_gt96100_init(vm, "gt96100", C3725_GT96K_ADDR, 0x200000,
                       C3725_GT96K_IRQ, C3725_EXT_IRQ,
                       c3725_net_irq_for_slot_port(0, 0), 255) == -1)
    return (-1);
...
static int c3725_init_platform(c3725_t *router) {
  ...
  if (!(obj =
            dev_flash_init(vm, "rom", C3725_ROM_ADDR, vm->rom_size * 1048576)))
    goto err;
  ...
  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C3725_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
...
/* Get MAC address MSB */
static u_int c3725_get_mac_addr_msb(void) { return (0xC2); }
