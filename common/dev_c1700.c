...
static int c1700_nvram_extract_config(vm_instance_t *vm,
...
static int c1700_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
...
static inline void c1700_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
int c1700_mainboard_set_type(c1700_t *router, char *mainboard_type) {
  ...
  /* Set the cookie */
  memcpy(router->vm->chassis_cookie, eeprom_c1700_mb_data,
         sizeof(eeprom_c1700_mb_data));
...
static int c1700_init_platform(c1700_t *router) {
  ...
  if (!vm->rom_filename) {
    /* use embedded ROM */
    dev_rom_init(vm, "rom", C1700_ROM_ADDR, 512 * 1024, ppc32_microcode,
                 ppc32_microcode_len);
  } else {
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C1700_ROM_ADDR,
                 512 * 1024);
  }
  ...
  /* NVRAM */
  dev_nvram_init(vm, "nvram", C1700_NVRAM_ADDR, vm->nvram_size * 1024,
                 &vm->conf_reg);
...
/* Get MAC address MSB */
static u_int c1700_get_mac_addr_msb(void) { return (0xD0); }
