...
/* Locate an entry */
static inline registry_entry_t *registry_find_entry(char *name,
                                                    int object_type) {
 ...
 * the specified count.
 */
int registry_exec_refcount(char *name, int object_type, int max_ref,
                           int reg_del, registry_exec obj_action,
                           void *opt_arg) {
  registry_entry_t *entry;
  int res = -1;
  int status;
...
int registry_delete_if_unused(char *name, int object_type,
  ...
  return (registry_exec_refcount(name, object_type, 0, TRUE, obj_destructor,
                                 opt_arg));
}

/* Execute a callback function for all objects of specified type */
int registry_foreach_type(int object_type, registry_foreach cb, void *opt,
                          int *err) {
...
int registry_delete_type(int object_type, registry_exec cb, void *opt) {
  ...
  for (p = bucket->htype_next; p != bucket; p = next) {
    ...
    if (p->ref_count == 0) {
      ...
      fprintf(stderr,
              "registry_delete_type: object \"%s\" (type %d) still "
              "referenced (count=%d)\n",
