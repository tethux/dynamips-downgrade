...
int fs_fat_format16(int fd, m_uint32_t begin_lba, m_uint32_t nr_sectors,
  ...
  if (!volume_name) {
    name[0] = 0;
    snprintf(name, sizeof(name), "DISK%dMB",
             (nr_sectors / (1048576 / FS_FAT_SECTOR_SIZE)));
    volume_name = name;
  }
  ...
  /* Root directory */
  rootdir_lba = begin_lba + fat16->reserved_sectors +
                (fat16->nr_fats * fat16->fat_sectors);
  rootdir_sectors =
      ((fat16->root_entry_count * 32) + (FS_FAT_SECTOR_SIZE - 1)) /
      FS_FAT_SECTOR_SIZE;
  fat16_empty(sector, fat16);
