...
/* Log a Flash message */
#define FLASH_LOG(d, msg...)                                                   \
  vm_log((d)->flash_set->vm, (d)->flash_set->dev.name, msg)
...
static struct flash_model flash_models[] = {
    ...
    {"c1700-bootflash-4mb", 4 * 1048576, FLASH_MODE_WORD, 0, 0x10000, 0x89,
     0x14},

    /* C1700 8 Mb bootflash: 1x28F640 in word mode */
    {"c1700-bootflash-8mb", 8 * 1048576, FLASH_MODE_WORD, 0, 0x10000, 0x89,
     0x15},

    /* C3600 8 Mb bootflash: 4x28F016SA in byte mode */
    {"c3600-bootflash-8mb", 8 * 1048576, FLASH_MODE_BYTE, 2, 0x10000, 0x89,
     0xA0},

    /* C7200 4 Mb bootflash: 4x28F008SA in byte mode */
    {"c7200-bootflash-4mb", 4 * 1048576, FLASH_MODE_BYTE, 2, 0x10000, 0x89,
     0xA2},

    /* C7200 8 Mb bootflash: 4x28F016SA in byte mode */
    {"c7200-bootflash-8mb", 8 * 1048576, FLASH_MODE_BYTE, 2, 0x10000, 0x89,
     0xA0},
     ...
     */
    {"c7200-bootflash-64mb", 64 * 1048576, FLASH_MODE_BYTE, 2, 0x10000, 0x89,
     0x18},

    /* C2600 8 Mb bootflash: 4x28F016SA in byte mode */
    {"c2600-bootflash-8mb", 8 * 1048576, FLASH_MODE_BYTE, 2, 0x10000, 0x89,
     0xA0},
...
static int flashset_init(struct flashset_data *d, u_int mode,
                         u_int nr_flash_bits, u_int blk_size,
...
static void flash_access(struct flash_data *d, m_uint32_t offset, u_int op_type,
  ...
  if (op_type == MTS_READ) {
    FLASH_LOG(d,
              "flash_access(%u): read  access to offset 0x%8.8x "
              "(state=%u)\n",
              d->flash_pos, offset, d->state);
  } else {
    FLASH_LOG(d,
              "flash_access(%u): write access to offset 0x%8.8x, "
...
#if DEBUG_WRITE
      FLASH_LOG(d,
                "flash_access(%u): erasing block at offset 0x%8.8x\n" offset);
#endif
      if ((*data & 0xFF) == 0xD0) {
