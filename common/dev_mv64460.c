...
static void mv64460_sdma_rxdesc_put_pkt(struct mv64460_data *d,
                                        struct sdma_desc *rxd, u_char **pkt,
                                        ssize_t *pkt_len) {
  ssize_t len, cp_len;
...
_Unused static int mv64460_sdma_handle_rx_pkt(netio_desc_t *nio, u_char *pkt,
                                              ssize_t pkt_len,
...
int mv64460_sdma_bind_vtty(struct mv64460_data *d, u_int chan_id,
                           vtty_t *vtty) {
...
static int mv64460_eth_handle_port_txqueue(struct mv64460_data *d,
                                           struct eth_port *port, int queue) {
  u_char pkt[MV64460_MAX_PKT_SIZE], *pkt_ptr;
  struct sdma_desc txd0, ctxd, *ptxd;
  m_uint32_t tx_start, tx_current;
  m_uint32_t len, tot_len;
...
#if DEBUG_ETH_TX
    MV64460_LOG(d,
                "mv64460_eth_handle_txqueue: loop: "
...
static void mv64460_eth_handle_port_txqueues(struct mv64460_data *d,
                                             u_int port) {
  int i;
...
static int mv64460_eth_handle_rxqueue(struct mv64460_data *d, u_int port_id,
  ...
  MV64460_LOG(d,
              ...
              "[buf_size=0x%8.8x,cmd_stat=0x%8.8x,"
              "next_ptr=0x%8.8x,buf_ptr=0x%8.8x]\n",
              port_id, queue, rx_start, rxd0.buf_size, rxd0.cmd_stat,
              rxd0.next_ptr, rxd0.buf_ptr);
#endif
...
static int mv64460_eth_handle_rx_pkt(netio_desc_t *nio, u_char *pkt,
                                     ssize_t pkt_len, struct mv64460_data *d,
                                     void *arg) {
...
static void mv64460_eth_mii_write(struct mv64460_data *d) {
  ...
  if (reg < 32) {
    ...
    MV64460_LOG(d, "MII: port 0x%4.4x, reg 0x%2.2x: writing 0x%4.4x\n", port,
                reg, d->smi_reg & MV64460_ETH_SMI_DATA_MASK);
#endif
...
int dev_mv64460_eth_set_nio(struct mv64460_data *d, u_int port_id,
  ...
  port->nio = nio;
  netio_rxl_add(nio, (netio_rx_handler_t)mv64460_eth_handle_rx_pkt, d,
                (void *)(u_long)port_id);
  return (0);
}
...
void *dev_mv64460_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
#if DEBUG_ACCESS
  if (op_type == MTS_READ) {
    cpu_log(cpu, "MV64460", "read access to register 0x%x, pc=0x%llx\n", offset,
            cpu_get_pc(cpu));
  } else {
    cpu_log(cpu, "MV64460",
  ...
  case MV64460_REG_CPU_INTN0_SEL_CAUSE:
    if (op_type == MTS_READ) {
      *data = mv64460_ic_get_sel_cause(mv_data, mv_data->cpu_intn0_mask_lo,
  ...
  case MV64460_REG_CPU_INTN1_SEL_CAUSE:
    if (op_type == MTS_READ) {
      *data = mv64460_ic_get_sel_cause(mv_data, mv_data->cpu_intn1_mask_lo,
  ...
  case MV64460_REG_INT0N_SEL_CAUSE:
    if (op_type == MTS_READ) {
      *data = mv64460_ic_get_sel_cause(mv_data, mv_data->int0n_mask_lo,
  ...
  case MV64460_REG_INT1N_SEL_CAUSE:
    if (op_type == MTS_READ) {
      *data = mv64460_ic_get_sel_cause(mv_data, mv_data->int1n_mask_lo,
  ...
  default:
    if (op_type == MTS_READ) {
      ...
      cpu_log(cpu, "MV64460",
              "write to addr 0x%x, value=0x%llx, "
              "pc=0x%llx\n",
              offset, *data, cpu_get_pc(cpu));
    }
#endif
...
int dev_mv64460_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
  ...
  if (!pci_dev_lookup(d->bus[0], 0, 0, 0)) {
    d->pci_dev = pci_dev_add(d->bus[0], name, PCI_VENDOR_MARVELL,
                             PCI_PRODUCT_MARVELL_MV64460, 0, 0, -1, d, NULL,
                             pci_mv64460_read, NULL);
    if (!d->pci_dev) {
  ...
  /* TEST */
  pci_dev_add(d->bus[1], name, PCI_VENDOR_MARVELL, PCI_PRODUCT_MARVELL_MV64460,
              0, 0, -1, d, NULL, pci_mv64460_read, NULL);

  /* Start the Ethernet TX ring scanner */
  d->eth_tx_tid =
      ptask_add((ptask_callback)mv64460_eth_handle_txqueues, d, NULL);
