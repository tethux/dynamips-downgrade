...
/* Store a packet */
static int dev_i8255x_store_rx_pkt(struct i8255x_data *d, m_uint8_t *pkt,
                                   ssize_t pkt_len) {
 ...
 * dev_i8255x_access()
 */
void *dev_i8255x_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
/* Handle the RX ring */
static int dev_i8255x_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                    ssize_t pkt_len, struct i8255x_data *d) {
  int res = FALSE;
 ...
 */
static void pci_i8255x_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                             m_uint32_t value) {
  struct i8255x_data *d = dev->priv_data;
 ...
 * dev_i8255x_init()
 */
struct i8255x_data *dev_i8255x_init(vm_instance_t *vm, char *name,
                                    int interface_type, struct pci_bus *pci_bus,
  ...
  /* Add as PCI device */
  pci_dev = pci_dev_add(pci_bus, name, I8255X_PCI_VENDOR_ID,
                        I8255X_PCI_PRODUCT_ID, pci_device, 0, irq, d, NULL,
                        pci_i8255x_read, pci_i8255x_write);
