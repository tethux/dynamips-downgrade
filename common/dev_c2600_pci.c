...
void *dev_c2600_pci_access(cpu_gen_t *cpu, struct vdevice *dev,
  ...
  if (!pci_dev) {
    if (op_type == MTS_READ) {
      ...
      cpu_log(cpu, "PCI",
              "write request (data=0x%8.8llx) for unknown device "
...
static void dev_c2600_pci_bridge_write(cpu_gen_t *cpu, struct pci_device *dev,
  ...
  case 0x10:
    ...
    if (value != 0x00000000) {
      vm_error(d->vm, "C2600_PCI", "Trying to set bridge BAR0 at 0x%8.8x!\n",
               value);
    }
    d->bridge_bar0 = value;
    break;
...
int dev_c2600_pci_init(vm_instance_t *vm, char *name, m_uint64_t paddr,
  ...
  pci_dev_add(d->bus, "pci_bridge", C2600_PCI_BRIDGE_VENDOR_ID,
              C2600_PCI_BRIDGE_PRODUCT_ID, 15, 0, -1, d, NULL,
              dev_c2600_pci_bridge_read, dev_c2600_pci_bridge_write);
