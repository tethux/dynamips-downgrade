...
static void bcm5600_reg_write_special(struct nm_16esw_data *d, m_uint32_t addr,
                                      m_uint32_t value) {
  switch (addr) {
  case 0x80006:
...
_Unused static int bcm5600_table_dump(struct nm_16esw_data *d,
                                      m_uint32_t addr) {
  struct bcm5600_table *table;
  m_uint32_t *entry;
  int i, j;
...
static inline int bcm5600_gen_arl_lookup(struct nm_16esw_data *d,
                                         ...
                                         n_eth_addr_t *mac_addr, u_int vlan) {
...
static inline int bcm5600_arl_lookup(struct nm_16esw_data *d,
  ...
  struct bcm5600_table *table = d->t_arl;
  return (
      bcm5600_gen_arl_lookup(d, table, 1, d->arl_cnt[0] - 1, mac_addr, vlan));
}
...
static inline int bcm5600_marl_lookup(struct nm_16esw_data *d,
  ...
  return (bcm5600_gen_arl_lookup(d, table, table->min_index,
                                 table->max_index + 1, mac_addr, vlan));
}
...
static int bcm5600_delete_arl_entry(struct nm_16esw_data *d) {
  ...
  for (i = table->min_index; i <= table->max_index; i++) {
    ...
    if ((cvlan == vlan) && (entry[0] == d->dw[1]) &&
      ...
      if (!(last_entry =
                bcm5600_table_get_entry(d, d->t_arl, d->arl_cnt[0] - 2)))
        continue;
...
#if DEBUG_ACCESS
  BCM_LOG(d,
          "gen_cmd: opcode 0x%2.2x [src=0x%2.2x,dst=0x%2.2x,len=0x%2.2x] "
...
static int bcm5600_send_pkt_to_cpu(struct nm_16esw_data *d,
  ...
  physmem_copy_u32_to_vm(d->vm, d->rx_current + 0x14,
                         0x40000000 + (pkt_len + 4));
...
static int bcm5600_trunk_egress_port(struct nm_16esw_data *d,
                                     struct bcm5600_pkt *p, u_int trunk_id) {
...
static int bcm5600_dst_mac_lookup(struct nm_16esw_data *d,
  ...
  if (!is_mcast) {
    ...
    p->egress_ut_bitmap = p->vlan_entry[2] & BCM5600_VTABLE_UT_PORT_BMAP_MASK;
  } else {
...
typedef void (*bcm5600_send_pkt_t)(struct nm_16esw_data *d,
                                   struct bcm5600_pkt *p, netio_desc_t *nio);
...
static void bcm5600_send_pkt_push_dot1q(struct nm_16esw_data *d,
  ...
  if (!p->rewrite_done) {
    ...
    memcpy(p->rewr_pkt + sizeof(n_eth_dot1q_hdr_t), p->pkt + (N_ETH_HLEN - 2),
...
static void bcm5600_send_pkt_pop_dot1q(struct nm_16esw_data *d,
  ...
  if (!p->rewrite_done) {
    ...
    memcpy(p->rewr_pkt + (N_ETH_HLEN - 2), p->pkt + sizeof(n_eth_dot1q_hdr_t),
...
static int bcm5600_handle_rx_pkt(struct nm_16esw_data *d,
                                 struct bcm5600_pkt *p) {
...
static int bcm5600_handle_tx_pkt(struct nm_16esw_data *d, struct bcm5600_pkt *p,
                                 u_int egress_bitmap) {
  n_eth_dot1q_hdr_t *eth_hdr;
...
/* Handle the RX ring */
static int dev_bcm5600_handle_rxring(netio_desc_t *nio, u_char *pkt,
                                     ssize_t pkt_len, struct nm_16esw_data *d,
                                     struct bcm5600_port *port) {
  struct bcm5600_pkt pkt_data;
  m_uint32_t rxd_len;
...
/* pci_bcm5605_write() */
static void pci_bcm5605_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                              m_uint32_t value) {
  struct nm_16esw_data *d = dev->priv_data;
...
struct nm_16esw_data *dev_nm_16esw_init(vm_instance_t *vm, char *name,
                                        u_int nm_bay, struct pci_bus *pci_bus,
  ...
  data->pci_dev = pci_dev_add(pci_bus, name, BCM5605_PCI_VENDOR_ID,
                              BCM5605_PCI_PRODUCT_ID, pci_device, 0, irq, data,
  ...
  data->tx_tid =
      ptask_add((ptask_callback)dev_bcm5600_handle_txring, data, NULL);

  /* Start the MAC address ager */
  data->ager_tid =
      timer_create_entry(15000, FALSE, 10, (timer_proc)bcm5600_arl_ager, data);
  return data;
}
