...
static inline void timer_remove_from_queue_atomic(timer_queue_t *queue,
                                                  timer_entry_t *timer) {
  TIMERQ_LOCK(queue);
...
static int timer_schedule(timer_entry_t *timer) {
  ...
  if (!(queue = timer_select_queue_from_pool())) {
    fprintf(stderr, "timer_schedule: no pool available for timer with ID %llu",
            timer->id);
    return (-1);
  }
...
timer_queue_t *timer_create_queue(void) {
  ...
  /* Create thread */
  if (pthread_create(&queue->thread, NULL, (void *(*)(void *))timer_loop,
                     queue))
    goto err_create;
