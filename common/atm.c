...
void atm_init(void) { gen_syndrome_table(); }
...
/* Receive an ATM cell */
static int atmsw_recv_cell(netio_desc_t *nio, u_char *atm_cell,
                           ssize_t cell_len, atmsw_table_t *t) {
  int res;
...
int atmsw_release(char *name) { return (registry_unref(name, OBJ_TYPE_ATMSW)); }
...
int atmsw_create_vpc(atmsw_table_t *t, char *nio_input, u_int vpi_in,
  ...
  if (netio_rxl_add(swc->input, (netio_rx_handler_t)atmsw_recv_cell, t, NULL) ==
      -1)
    goto error;
...
int atmsw_delete_vpc(atmsw_table_t *t, char *nio_input, u_int vpi_in,
  ...
  for (swc = &t->vp_table[hbucket]; *swc; swc = &(*swc)->next) {
    ...
    if ((p->input == input) && (p->output == output) && (p->vpi_in == vpi_in) &&
        (p->vpi_out == vpi_out)) {
...
int atmsw_create_vcc(atmsw_table_t *t, char *input, u_int vpi_in, u_int vci_in,
  ...
  if (atmsw_vp_lookup(t, swc->input, vpi_in) != NULL) {
    fprintf(stderr,
            "atmsw_create_vcc: VP switching already exists for "
            "VPI=%u\n",
            vpi_in);
    goto error;
  }
...
int atmsw_delete_vcc(atmsw_table_t *t, char *nio_input, u_int vpi_in,
                     u_int vci_in, char *nio_output, u_int vpi_out,
                     u_int vci_out) {
  netio_desc_t *input, *output;
  ...
  for (swc = &t->vc_table[hbucket]; *swc; swc = &(*swc)->next) {
    ...
    if ((p->input == input) && (p->output == output) && (p->vpi_in == vpi_in) &&
        (p->vci_in == vci_in) && (p->vpi_out == vpi_out) &&
        (p->vci_out == vci_out)) {
      /* found a matching VP, remove it */
      *swc = (*swc)->next;
      ATMSW_UNLOCK(t);
...
void atmsw_save_config(atmsw_table_t *t, FILE *fd) {
  ...
  for (i = 0; i < ATMSW_VP_HASH_SIZE; i++) {
    for (vp = t->vp_table[i]; vp; vp = vp->next) {
      fprintf(fd, "atmsw create_vpc %s %s %u %s %u\n", t->name, vp->input->name,
              vp->vpi_in, vp->output->name, vp->vpi_out);
    }
  }
  ...
  for (i = 0; i < ATMSW_VC_HASH_SIZE; i++) {
    for (vc = t->vc_table[i]; vc; vc = vc->next) {
      fprintf(fd, "atmsw create_vcc %s %s %u %u %s %u %u\n", t->name,
              vc->input->name, vc->vpi_in, vc->vci_in, vc->output->name,
              vc->vpi_out, vc->vci_out);
    }
  }
...
static void atmsw_reg_save_config(registry_entry_t *entry, void *opt,
                                  int *err) {
...
int atmsw_cfg_create_if(atmsw_table_t *t, char **tokens, int count) {
  ...
  case NETIO_TYPE_UNIX:
    if (count != 5) {
      fprintf(stderr,
              "ATMSW: invalid number of arguments "
              "for UNIX NIO '%s'\n",
              tokens[1]);
      break;
    }
  ...
  case NETIO_TYPE_UDP:
    ...
    nio = netio_desc_create_udp(tokens[1], atoi(tokens[3]), tokens[4],
                                atoi(tokens[5]));
    break;
  ...
  if (!nio) {
    fprintf(stderr,
            "ATMSW: unable to create NETIO descriptor of "
            "interface %s\n",
            tokens[1]);
    return (-1);
  }
...
int atmsw_cfg_create_vpc(atmsw_table_t *t, char **tokens, int count) {
  ...
  return (atmsw_create_vpc(t, tokens[1], atoi(tokens[2]), tokens[3],
                           atoi(tokens[4])));
}
