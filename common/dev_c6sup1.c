...
/* Remove EOBC */
static int dev_c6sup1_eobc_shutdown(vm_instance_t *vm,
...
static int dev_c6sup1_eobc_unset_nio(vm_instance_t *vm, struct cisco_card *card,
                                     u_int port_id) {
  struct am79c971_data *d = card->drv_info;
...
/* Unbind a Network IO descriptor */
static int dev_c6sup1_ibc_unset_nio(vm_instance_t *vm, struct cisco_card *card,
                                    u_int port_id) {
  struct i8254x_data *d = card->drv_info;
...
static int c6sup1_nvram_extract_config(vm_instance_t *vm,
...
static int c6sup1_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
...
static inline void c6sup1_net_irq_get_slot_port(u_int irq, u_int *slot,
                                                u_int *port) {
...
static int c6sup1_init_platform(c6sup1_t *router) {
  ...
  /* NVRAM and calendar */
  dev_nvram_init(vm, "nvram", C6SUP1_NVRAM_ADDR, vm->nvram_size * 1024,
                 &vm->conf_reg);
  ...
  if (!vm->rom_filename) {
    ...
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C6SUP1_ROM_ADDR,
                 vm->rom_size * 1048576);
  }

  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C6SUP1_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
...
/* Get MAC address MSB */
static u_int c6sup1_get_mac_addr_msb(void) { return (0xD0); }
