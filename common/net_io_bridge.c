...
/* Receive a packet */
static int netio_bridge_recv_pkt(netio_desc_t *nio, u_char *pkt,
                                 ssize_t pkt_len, netio_bridge_t *t) {
  int i;
...
static void netio_bridge_reg_save_config(registry_entry_t *entry, void *opt,
                                         int *err) {
  netio_bridge_save_config((netio_bridge_t *)entry->data, (FILE *)opt);
}

void netio_bridge_save_config_all(FILE *fd) {
  registry_foreach_type(OBJ_TYPE_NIO_BRIDGE, netio_bridge_reg_save_config, fd,
                        NULL);
}
...
static int netio_bridge_cfg_create_if(netio_bridge_t *t, char **tokens,
                                      int count) {
  ...
  case NETIO_TYPE_UDP:
    ...
    nio = netio_desc_create_udp(tokens[0], atoi(tokens[2]), tokens[3],
                                atoi(tokens[4]));
    break;
