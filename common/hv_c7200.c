...
static int cmd_set_npe(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if ((c7200_npe_set_type(VM_C7200(vm), argv[1])) == -1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to set NPE type for router '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_get_mac_addr(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  hypervisor_send_reply(
      conn, HSC_INFO_OK, 1, "%2.2x%2.2x.%2.2x%2.2x.%2.2x%2.2x",
      router->mac_addr.eth_addr_byte[0], router->mac_addr.eth_addr_byte[1],
      router->mac_addr.eth_addr_byte[2], router->mac_addr.eth_addr_byte[3],
      router->mac_addr.eth_addr_byte[4], router->mac_addr.eth_addr_byte[5]);
...
static int cmd_set_mac_addr(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if ((c7200_midplane_set_mac_addr(VM_C7200(vm), argv[1])) == -1) {
    vm_release(vm);
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to set MAC address for router '%s'", argv[0]);
    return (-1);
  }
 ...
 */
static int cmd_set_temp_sensor(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  vm_instance_t *vm;
  c7200_t *router;
  u_int index;
 ...
 */
static int cmd_set_power_supply(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
  c7200_t *router;
  u_int index;
