...
void *dev_pa_a1_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
  ...
  if (op_type == MTS_READ) {
    ...
    cpu_log(cpu, "TI1570",
            "write access to vaddr = 0x%x, pc = 0x%llx, "
            "val = 0x%llx\n",
...
static void ti1570_update_aal5_crc(struct pa_a1_data *d,
                                   ti1570_tx_dma_entry_t *tde) {
  tde->aal5_crc = crc32_compute(~tde->aal5_crc, &d->txfifo_cell[ATM_HDR_SIZE],
                                ATM_PAYLOAD_SIZE);
}
...
static void ti1570_add_tx_padding(struct pa_a1_data *d, m_uint32_t len) {
  if (len > d->txfifo_avail) {
    TI1570_LOG(d,
               "ti1570_add_tx_padding: trying to add too large "
...
static m_uint32_t ti1570_init_tx_atm_cell(struct pa_a1_data *d,
...
#if DEBUG_TRANSMIT
  TI1570_LOG(d,
             "ti1570_init_tx_atm_cell: data ptr=0x%x, "
  ...
  if (len > 0) {
    physmem_copy_from_vm(d->vm, &d->txfifo_cell[d->txfifo_pos], tde->cb_addr,
                         len);
    d->txfifo_pos += len;
    d->txfifo_avail -= len;
  }
...
static int ti1570_transmit_aal5_cell(struct pa_a1_data *d,
                                     ti1570_tx_dma_entry_t *tde, int *buf_end) {
  m_uint32_t buf_size, len;
  int pkt_end;
...
#if DEBUG_TRANSMIT
  TI1570_LOG(d,
             "ti1570_transmit_aal5_cell: data ptr=0x%x, "
             "buf_size=0x%x (%u)\n",
...
static int ti1570_scan_tx_dma_entry_single(struct pa_a1_data *d,
  ...
  TI1570_LOG(d,
             "ti1570_scan_tx_dma_entry: TX DMA entry %u is ON "
             "(ctrl_buf = 0x%x)\n",
             index, tde->ctrl_buf);
#endif
...
#if DEBUG_TX_DMA
    TI1570_LOG(d,
               "ti1570_scan_tx_dma_entry: psr_addr = 0x%x, "
               "psr_entry = 0x%x\n",
               psr_addr, psr_entry);
#endif
...
#if DEBUG_TX_DMA
      TI1570_LOG(d,
                 "ti1570_scan_tx_dma_entry: PSR ring rotation "
                 "(psr_end = %u)\n",
                 psr_end);
#endif
    }
...
static void ti1570_scan_tx_sched_table(struct pa_a1_data *d) {
  ...
  for (i = 0; i < TI1570_TX_SCHED_ENTRY_COUNT >> 1; i++) {
    ...
    if (index0)
      ti1570_scan_tx_dma_entry(d, index0);
    if (index1)
      ti1570_scan_tx_dma_entry(d, index1);
  }
}
...
static void ti1570_update_rx_cring(struct pa_a1_data *d,
                                   ...
                                   m_uint32_t atm_hdr, m_uint32_t aal5_trailer,
                                   m_uint32_t err_ind, m_uint32_t fbuf_valid) {
  ...
  if (err_ind)
    rcre.fbr_entry |= TI1570_RCR_ERROR;
...
#if DEBUG_RECEIVE
    TI1570_LOG(d,
               "ti1570_acquire_rx_buffer: acquiring non-FIFO buffer, "
               "ring index=%u (0x%x)\n",
...
#if DEBUG_RECEIVE
    TI1570_LOG(d,
               "ti1570_acquire_rx_buffer: ring size=%u (0x%x), "
               "buf size=%u ATM cells\n",
               ring_size, ring_size, buf_size);

    TI1570_LOG(d,
               "ti1570_acquire_rx_buffer: buffer index=%u (0x%x), "
...
#if DEBUG_RECEIVE
      TI1570_LOG(d,
                 "ti1570_acquire_rx_buffer: buf_idx=0x%x, "
...
#if DEBUG_RECEIVE
  TI1570_LOG(d,
             "ti1570_store_rx_cell: storing cell payload at 0x%x "
             "(buf_addr=0x%x)\n",
             rde->cb_addr, rde->sb_addr << 2);
#endif

  physmem_copy_to_vm(d->vm, &atm_cell[ATM_HDR_SIZE], rde->cb_addr,
                     ATM_PAYLOAD_SIZE);
...
#if DEBUG_RECEIVE
  TI1570_LOG(d,
             "ti1570_store_rx_cell: new rde->cb_len = 0x%x, "
             "buf_len=0x%x\n",
             rde->cb_len, buf_len);
#endif
...
static int ti1570_handle_rx_cell(netio_desc_t *nio, u_char *atm_cell,
                                 ssize_t cell_len, struct pa_a1_data *d) {
  ...
  if (!(rvd_entry & TI1570_RX_VPI_ENABLE)) {
    TI1570_LOG(d,
               "ti1570_handle_rx_cell: received cell with "
               "unknown VPI %u (VCI=%u)\n",
               vpi, vci);
    return (FALSE);
  }
  ...
  if ((vci == 3) || (vci == 4))
    ...
    if ((atm_hdr & ATM_PTI_NETWORK) != 0) {
      ...
      if (!(atm_hdr & ATM_PTI_NETWORK)) {
        ...
        if (vci_idx > vci_max) {
          TI1570_LOG(d,
                     "ti1570_handle_rx_cell: out-of-range VCI %u "
...
#if DEBUG_RECEIVE
        TI1570_LOG(d,
                   "ti1570_handle_rx_cell: VPI/VCI=%u/%u, "
        ...
        if (bptr < TI1570_RX_DMA_TABLE_OFFSET) {
          TI1570_LOG(d,
                     "ti1570_handle_rx_cell: inconsistency in "
 ...
 * pci_ti1570_write()
 */
static void pci_ti1570_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                             m_uint32_t value) {
  struct pa_a1_data *d = dev->priv_data;
 ...
 * pci_plx9060es_write()
 */
static void pci_plx9060es_write(cpu_gen_t *cpu, struct pci_device *dev, int reg,
                                m_uint32_t value) {
  // struct pa_a1_data *d = dev->priv_data;
...
int dev_c7200_pa_a1_init(vm_instance_t *vm, struct cisco_card *card) {
  ...
  pci_dev_ti = pci_dev_add(card->pci_bus, card->dev_name, TI1570_PCI_VENDOR_ID,
                           TI1570_PCI_PRODUCT_ID, 0, 0,
                           c7200_net_irq_for_slot_port(slot, 0), d, NULL,
                           pci_ti1570_read, pci_ti1570_write);
  ...
  pci_dev_plx =
      pci_dev_add(card->pci_bus, card->dev_name, PLX_9060ES_PCI_VENDOR_ID,
                  PLX_9060ES_PCI_PRODUCT_ID, 1, 0, -1, d, NULL,
                  pci_plx9060es_read, pci_plx9060es_write);
