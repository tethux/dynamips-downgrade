...
struct pci_io_device *pci_io_add(struct pci_io_data *d, m_uint32_t start,
                                 m_uint32_t end, struct vdevice *dev,
...
static void *pci_io_access(cpu_gen_t *cpu, struct vdevice *dev,
  ...
  for (p = d->dev_list; p; p = p->next)
    if ((offset >= p->start) && (offset <= p->end)) {
      return (p->handler(cpu, p->real_dev, (offset - p->start), op_size,
                         op_type, data));
    }
