 ...
 * dev_sb1_pci_access()
 */
void *dev_sb1_pci_access(cpu_gen_t *cpu, struct vdevice *dev, m_uint32_t offset,
...
int dev_sb1_pci_init(vm_instance_t *vm, char *name, m_uint64_t paddr) {
  ...
  d->pci_cfg_dev =
      pci_dev_add(d->pci_bus, "sb1_pci_cfg", SB1_PCI_VENDOR_ID, 0x0001, 0, 0,
                  -1, NULL, NULL, sb1_pci_cfg_read, NULL);
  ...
  d->ht_cfg_dev =
      pci_bridge_create_dev(d->pci_bus, "sb1_ht_cfg", SB1_PCI_VENDOR_ID, 0x0002,
                            1, 0, vm->pci_bus_pool[28], sb1_ht_cfg_read, NULL);
