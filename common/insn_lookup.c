...
static inline void cbm_bitwise_and(cbm_array_t *result, cbm_array_t *a1,
                                   cbm_array_t *a2) {
  int i;
...
static void rfc_free_array_cbm_hash_value(void *key, void *value,
                                          void *opt_arg) {
...
insn_lookup_t *ilt_create(char *table_name, m_uint32_t crc32_insn, int nr_insn,
  ...
  if ((ilt = ilt_cache_load(table_name))) {
    if (ilt->crc32_insn == crc32_insn && ilt->nr_insn == nr_insn &&
