...
static int c2600_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(vm, "nvram", vm->nvram_rom_space * 4, 0, 0,
                                     FS_NVRAM_FORMAT_SCALE_4, startup_config,
...
static int c2600_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
                                   size_t startup_len, u_char *private_config,
                                   size_t private_len) {
  int ret;

  ret = generic_nvram_push_config(vm, "nvram", vm->nvram_size * 4096,
                                  vm->nvram_rom_space * 4, 0, 0,
                                  FS_NVRAM_FORMAT_SCALE_4, startup_config,
                                  startup_len, private_config, private_len);
...
static inline void c2600_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
...
int c2600_mainboard_set_type(c2600_t *router, char *mainboard_type) {
  ...
  /* Set the cookie */
  memcpy(router->vm->chassis_cookie, eeprom_c2600_mb_data,
         sizeof(eeprom_c2600_mb_data));
...
static int c2600_init_platform(c2600_t *router) {
  ...
  if (!vm->rom_filename) {
    /* use embedded ROM */
    dev_rom_init(vm, "rom", C2600_ROM_ADDR, 512 * 1024, ppc32_microcode,
                 ppc32_microcode_len);
  } else {
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C2600_ROM_ADDR,
                 512 * 1024);
  }

  /* RAM aliasing */
  dev_create_ram_alias(vm, "ram_alias", "ram", 0x80000000,
                       vm->ram_size * 1048576);

  /* NVRAM */
  dev_ram_init(vm, "nvram", TRUE, FALSE, NULL, FALSE, C2600_NVRAM_ADDR,
               vm->nvram_size * 4096);
  c2600_nvram_check_empty_config(vm);

  /* Bootflash */
  dev_bootflash_init(vm, "flash0", "c2600-bootflash-8mb", C2600_FLASH_ADDR);
...
/* Get MAC address MSB */
static u_int c2600_get_mac_addr_msb(void) { return (0xC8); }
...
int c2600_platform_register(void) {
  return (vm_platform_register(&c2600_platform));
}
