...
/* NPE Drivers                                                              */
/* ======================================================================== */
#define DECLARE_NPE(type) int(c7200_init_##type)(c7200_t * router)
...
static int c7200_nvram_extract_config(vm_instance_t *vm,
  ...
  ret = generic_nvram_extract_config(vm, "nvram", vm->nvram_rom_space, 0,
                                     VM_C7200(vm)->npe_driver->nvram_addr +
                                         vm->nvram_rom_space,
                                     FS_NVRAM_FORMAT_ABSOLUTE, startup_config,
                                     startup_len, private_config, private_len);

  return (ret);
}

/* Directly push the IOS configuration to the NVRAM device */
static int c7200_nvram_push_config(vm_instance_t *vm, u_char *startup_config,
  ...
  ret = generic_nvram_push_config(
      ...
      VM_C7200(vm)->npe_driver->nvram_addr + vm->nvram_rom_space,
      FS_NVRAM_FORMAT_ABSOLUTE, startup_config, startup_len, private_config,
      private_len);
...
static int c7200_burn_mac_addr(c7200_t *router, n_eth_addr_t *addr) {
  ...
  if (eeprom_ver != 1) {
    vm_error(router->vm,
             "c7200_burn_mac_addr: unable to handle "
             "EEPROM version %u\n",
             eeprom_ver);
    return (-1);
  }
...
static inline void c7200_net_irq_get_slot_port(u_int irq, u_int *slot,
                                               u_int *port) {
 ...
 * BTW, it works.
 */
static int c7200_create_io_pci_bridge(c7200_t *router,
...
static int c7200m_init_platform(c7200_t *router) {
  ...
  if (vm->ram_size > router->npe_driver->max_ram_size) {
    vm_error(vm,
             "%u is not a valid RAM size for this NPE. "
             "Fallback to %u Mb.\n\n",
  ...
  if (!vm->rom_filename) {
    ...
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C7200_ROM_ADDR,
                 vm->rom_size * 1048576);
  }

  /* Byte swapping */
  dev_bswap_init(vm, "mem_bswap", C7200_BSWAP_ADDR, 1024 * 1048576,
                 0x00000000ULL);
  ...
  dev_clpd6729_init(vm, router->pcmcia_bus,
                    router->npe_driver->clpd6729_pci_dev, vm->pci_io_space,
                    0x402, 0x403);
...
static int c7200p_init_platform(c7200_t *router) {
  ...
  if (!vm->rom_filename) {
    ...
    /* use alternate ROM */
    dev_ram_init(vm, "rom", TRUE, TRUE, NULL, FALSE, C7200_G2_ROM_ADDR,
                 vm->rom_size * 1048576);
  }
...
static int c7200p_init_instance(c7200_t *router) {
  ...
  ppc32_map_zone(cpu0, cpu0->sr[C7200_G2_BOOTFLASH_ADDR >> 28],
                 C7200_G2_BOOTFLASH_ADDR, C7200_G2_BOOTFLASH_ADDR, 64 * 1048576,
                 0, 0x02);

  ppc32_map_zone(cpu0, cpu0->sr[0xD8000000 >> 28], 0xD8000000, 0xD8000000,
                 0x400000, 0, 0x02);
  ppc32_map_zone(cpu0, cpu0->sr[0xDC000000 >> 28], 0xDC000000, 0xDC000000,
                 0x400000, 0, 0x02);

  /* FIXME */
  ppc32_map_zone(cpu0, cpu0->sr[0xDF000000 >> 28], 0xDF000000, 0xDF000000,
                 0x400000, 0, 0x02);
...
/* Get MAC address MSB */
static u_int c7200_get_mac_addr_msb(void) { return (0xCA); }
