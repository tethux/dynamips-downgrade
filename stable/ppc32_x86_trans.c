...
static void ppc32_try_direct_far_jump(cpu_ppc_t *cpu, jit_op_t *iop,
  ...
  x86_mov_reg_membase(iop->ob_ptr, X86_EBX, X86_EDI,
                      OFFSET(cpu_ppc_t, exec_blk_map), 4);
  x86_mov_reg_membase(iop->ob_ptr, X86_EDX, X86_EBX, ia_hash * sizeof(void *),
                      4);
  ...
  /* Jump to the code */
  x86_mov_reg_membase(iop->ob_ptr, X86_ESI, X86_EDX,
                      OFFSET(ppc32_jit_tcb_t, jit_insn_ptr), 4);
  x86_mov_reg_membase(iop->ob_ptr, X86_EBX, X86_ESI, ia_offset * sizeof(void *),
                      4);
...
static int ppc32_jit_get_tmp_hreg(cpu_ppc_t *cpu) { return (X86_EBX); }
...
static void ppc32_emit_memop_fast(cpu_ppc_t *cpu, ppc32_jit_tcb_t *b,
  ...
  _maybe_used u_char *p_fast_exit;
  jit_op_t *iop;
  ...
  /* EDX = mts32_entry */
  x86_mov_reg_membase(iop->ob_ptr, X86_EDX, X86_EDI,
                      OFFSET(cpu_ppc_t, mts_cache[PPC32_MTS_DCACHE]), 4);
  ...
  x86_alu_reg_imm(iop->ob_ptr, X86_AND, X86_EBX, PPC32_MIN_PAGE_IMASK);
  x86_mov_reg_membase(iop->ob_ptr, X86_EAX, X86_EDX, OFFSET(mts32_entry_t, hpa),
                      4);
...
DECLARE_INSN(MFTBL) {
  ...
  x86_mov_reg_membase(iop->ob_ptr, hreg_rd, X86_EDI, OFFSET(cpu_ppc_t, tb), 4);
  x86_mov_reg_membase(iop->ob_ptr, hreg_t0, X86_EDI, OFFSET(cpu_ppc_t, tb) + 4,
                      4);
  ...
  x86_mov_membase_reg(iop->ob_ptr, X86_EDI, OFFSET(cpu_ppc_t, tb) + 4, hreg_t0,
                      4);
...
DECLARE_INSN(ADDC) {
  ...
  x86_mov_membase_reg(iop->ob_ptr, X86_EDI, OFFSET(cpu_ppc_t, xer_ca), hreg_t0,
                      4);
...
DECLARE_INSN(ADDE) {
  ...
  x86_alu_reg_membase(iop->ob_ptr, X86_ADD, hreg_t0, X86_EDI,
                      OFFSET(cpu_ppc_t, xer_ca));
  x86_set_reg(iop->ob_ptr, X86_CC_C, hreg_t1, FALSE);
  x86_mov_membase_reg(iop->ob_ptr, X86_EDI, OFFSET(cpu_ppc_t, xer_ca), hreg_t1,
                      4);
...
DECLARE_INSN(ADDIC) {
  ...
  x86_set_membase(iop->ob_ptr, X86_CC_C, X86_EDI, OFFSET(cpu_ppc_t, xer_ca),
                  FALSE);
...
DECLARE_INSN(ADDZE) {
  ...
  x86_alu_reg_membase(iop->ob_ptr, X86_ADD, hreg_rd, X86_EDI,
                      OFFSET(cpu_ppc_t, xer_ca));
...
DECLARE_INSN(BCC) {
  ...
  x86_test_membase_imm(iop->ob_ptr, X86_EDI, PPC32_CR_FIELD_OFFSET(cr_field),
                       (1 << cr_bit));
...
DECLARE_INSN(CRAND) {
  ...
  /* test $ba bit */
  x86_test_membase_imm(iop->ob_ptr, X86_EDI,
  ...
  x86_alu_membase_imm(iop->ob_ptr, X86_AND, X86_EDI,
  ...
  x86_alu_membase_reg(iop->ob_ptr, X86_OR, X86_EDI,
                      PPC32_CR_FIELD_OFFSET(ppc32_get_cr_field(bd)), hreg_t0);
...
DECLARE_INSN(MCRF) {
  ...
  x86_mov_reg_membase(iop->ob_ptr, hreg_t0, X86_EDI, PPC32_CR_FIELD_OFFSET(rs),
                      4);

  /* Store it in "rd" field */
  x86_mov_membase_reg(iop->ob_ptr, X86_EDI, PPC32_CR_FIELD_OFFSET(rd), hreg_t0,
                      4);
...
DECLARE_INSN(MFCR) {
  ...
  for (i = 0; i < 8; i++) {
    /* load field in %edx */
    x86_mov_reg_membase(iop->ob_ptr, hreg_t0, X86_EDI, PPC32_CR_FIELD_OFFSET(i),
                        4);
...
DECLARE_INSN(MFSR) {
  ...
  x86_mov_reg_membase(iop->ob_ptr, hreg_rd, X86_EDI,
