...
static int cmd_create(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (!(vm = vm_create_instance(argv[0], atoi(argv[1]), argv[2]))) {
    hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                          "unable to create VM instance '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_rename(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (registry_exists(argv[1], OBJ_TYPE_VM)) {
    ...
    hypervisor_send_reply(
        conn, HSC_ERR_RENAME, 1,
        "unable to rename VM instance '%s', '%s' already exists", argv[0],
        argv[1]);
    return (-1);
  }
  ...
  if (vm_rename_instance(vm, argv[1])) {
    hypervisor_send_reply(conn, HSC_ERR_RENAME, 1,
                          "unable to rename VM instance '%s'", argv[0]);
    return (-1);
  }

  vm_release(vm);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "VM '%s' renamed to '%s'",
                        argv[0], argv[1]);
  return (0);
}
...
static int cmd_delete(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (res == 1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_DELETE, 1, "unable to delete VM '%s'",
                          argv[0]);
  }
...
static int cmd_clean_delete(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (res == 1) {
    ...
    hypervisor_send_reply(conn, HSC_INFO_OK, 1,
                          "VM '%s' and related files deleted", argv[0]);
  } else {
    hypervisor_send_reply(conn, HSC_ERR_DELETE, 1, "unable to delete VM '%s'",
                          argv[0]);
  }
...
static int cmd_start(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (vm_init_instance(vm) == -1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_START, 1,
                          "unable to start VM instance '%s'", argv[0]);
    return (-1);
  }
...
static int cmd_stop(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (vm_stop_instance(vm) == -1) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_STOP, 1,
                          "unable to stop VM instance '%s'", argv[0]);
    return (-1);
  }
...
/* Set debugging level */
static int cmd_set_debug_level(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  vm_instance_t *vm;
...
static int cmd_set_ios(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  vm_release(vm);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "IOS image set for '%s'",
                        argv[0]);
  return (0);
}
...
static int cmd_set_clock_divisor(hypervisor_conn_t *conn, int argc,
                                 char *argv[]) {
  vm_instance_t *vm;
  u_int clock_div;
...
static int cmd_set_blk_direct_jump(hypervisor_conn_t *conn, int argc,
                                   char *argv[]) {
  vm_instance_t *vm;
...
static int cmd_set_idle_pc_online(hypervisor_conn_t *conn, int argc,
                                  char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
...
static int cmd_get_idle_pc_prop(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
  int i;
  ...
  for (i = 0; i < cpu->idle_pc_prop_count; i++) {
    hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "0x%llx [%d]",
                          cpu->idle_pc_prop[i].pc, cpu->idle_pc_prop[i].count);
  }
...
static int cmd_show_idle_pc_prop(hypervisor_conn_t *conn, int argc,
                                 char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
  int i;
...
static int cmd_set_idle_sleep_time(hypervisor_conn_t *conn, int argc,
                                   char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
...
static int cmd_show_timer_drift(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
  cpu_gen_t *cpu;
...
static int cmd_set_ghost_status(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
...
static int cmd_set_con_tcp_port(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
...
static int cmd_set_aux_tcp_port(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
...
static int cmd_extract_config(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if ((vm->platform->nvram_extract_config(vm, &startup_config, &startup_len,
                                          &private_config, &private_len)))
    goto err_nvram_extract;

  /*
   * Convert config to base64. base64 generates 4 bytes for each group of 3
   * bytes.
   */
  ...
  vm_release(vm);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1, "conf '%s' '%s' '%s'", argv[0],
                        startup_base64, private_base64);
...
static int cmd_push_config(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  /*
   * Convert base64 input to standard text. base64 uses 4 bytes for each group
   * of 3 bytes.
   */
  if (strcmp(argv[1], "(keep)") != 0) {
    ...
    if ((startup_len =
             base64_decode(startup_config, (u_char *)argv[1], startup_len)) < 0)
  ...
  if (argc > 2 && strcmp(argv[2], "(keep)") != 0) {
    ...
    if ((private_len =
             base64_decode(private_config, (u_char *)argv[2], private_len)) < 0)
      goto err_decode_base64;
  ...
  /* Push configuration */
  if (vm->platform->nvram_push_config(vm, startup_config, (size_t)startup_len,
                                      private_config, (size_t)private_len) < 0)
    goto err_nvram_push;
  ...
  vm_release(vm);
  hypervisor_send_reply(conn, HSC_INFO_OK, 1,
                        "IOS config file pushed tm VM '%s'", argv[0]);
  return (0);
...
err_no_push_method:
  vm_release(vm);
  hypervisor_send_reply(conn, HSC_ERR_CREATE, 1,
                        "unable to push IOS config for VM '%s'", argv[0]);
  return (-1);
}
...
static int cmd_send_con_msg(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (strcmp(format, "plain") == 0 && len >= 0) {
    ...
    len = base64_decode((unsigned char *)data, (const unsigned char *)argv[1],
                        len);
...
static int cmd_slot_bindings(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  for (i = 0; i < vm->nr_slots; i++) {
    ...
    /* main module */
    hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "%u/%u: %s", card->slot_id,
                          card->subslot_id, card->dev_type);
...
static int cmd_slot_nio_bindings(hypervisor_conn_t *conn, int argc,
                                 char *argv[]) {
  ...
  if ((card = vm_slot_get_card_ptr(vm, slot))) {
    ...
    for (nb = card->nio_list; nb; nb = nb->next) {
      hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "%u: %s", nb->port_id,
                            nb->nio->name);
    }
...
/* Add a slot binding */
static int cmd_slot_add_binding(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
...
static int cmd_slot_remove_binding(hypervisor_conn_t *conn, int argc,
                                   char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
...
static int cmd_slot_add_nio_binding(hypervisor_conn_t *conn, int argc,
                                    char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
...
static int cmd_slot_remove_nio_binding(hypervisor_conn_t *conn, int argc,
                                       char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
...
static int cmd_slot_enable_nio(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
  ...
  if (vm_slot_enable_nio(vm, slot, port) == -1) {
    vm_release(vm);
    hypervisor_send_reply(conn, HSC_ERR_BINDING, 1,
                          "VM %s: unable to enable NIO for slot %u/%u", argv[0],
                          slot, port);
    return (-1);
  }
...
static int cmd_slot_disable_nio(hypervisor_conn_t *conn, int argc,
                                char *argv[]) {
  vm_instance_t *vm;
  u_int slot, port;
...
static int cmd_slot_oir_start(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  if (vm_oir_start(vm, slot, subslot) == -1) {
    vm_release(vm);
    hypervisor_send_reply(conn, HSC_ERR_START, 1,
                          "VM %s: unable to engage OIR for slot %u/%u", argv[0],
                          slot, subslot);
    return (-1);
  }
...
static void cmd_show_vm_list(registry_entry_t *entry, void *opt, int *err) {
  ...
  hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "%s (%s)", entry->name,
                        vm_get_type(vm));
}
...
static void cmd_show_vm_list_con_ports(registry_entry_t *entry, void *opt,
  ...
  if (vm->vtty_con_type == VTTY_TYPE_TCP)
    hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "%s (%d)", vm->name,
                          vm->vtty_con_tcp_port);
}

/* VM console TCP port list */
static int cmd_vm_list_con_ports(hypervisor_conn_t *conn, int argc,
                                 char *argv[]) {
  int err = 0;
