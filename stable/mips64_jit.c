...
int mips64_jit_init(cpu_mips_t *cpu) {
  ...
  if (!cpu->exec_page_area) {
    fprintf(stderr, "mips64_jit_init: unable to create exec area (size %lu)\n",
...
int mips64_jit_tcb_record_patch(mips64_jit_tcb_t *block, u_char *jit_ptr,
  ...
  if (vaddr & 0x03) {
    fprintf(stderr,
            "Block 0x%8.8llx: trying to record an invalid PC "
...
static inline mips64_jit_tcb_t *mips64_jit_tcb_compile(cpu_mips_t *cpu,
                                                       m_uint64_t vaddr) {
  mips64_jit_tcb_t *block;
  struct mips64_insn_tag *tag;
...
static forced_inline void mips64_jit_tcb_run(cpu_mips_t *cpu,
                                             mips64_jit_tcb_t *block) {
#if DEBUG_SYM_TREE
