...
static int cmd_parser_test(hypervisor_conn_t *conn, int argc, char *argv[]) {
  ...
  for (i = 0; i < argc; i++)
    hypervisor_send_reply(conn, HSC_INFO_MSG, 0, "arg %d (len %lu): \"%s\"", i,
...
static int cmd_set_working_dir(hypervisor_conn_t *conn, int argc,
                               char *argv[]) {
  if (chdir(argv[0]) == -1) {
    hypervisor_send_reply(conn, HSC_ERR_INV_PARAM, 1, "chdir: %s",
                          strerror(errno));
  } else {
...
void *hypervisor_find_object(hypervisor_conn_t *conn, char *name,
                             int obj_type) {
  void *p;
...
void *hypervisor_find_vm(hypervisor_conn_t *conn, char *name) {
  ...
  if (!(vm = vm_acquire(name))) {
    hypervisor_send_reply(conn, HSC_ERR_UNK_OBJ, 1, "unable to find VM '%s'",
                          name);
    return NULL;
  }
  ...
  if (vm->platform != platform) {
    ...
    hypervisor_send_reply(conn, HSC_ERR_BAD_OBJ, 1,
                          "VM '%s' is not a VM type %s", name, platform->name);
    return NULL;
  }
...
static int hypervisor_exec_cmd(hypervisor_conn_t *conn, char *mod_name,
                               char *cmd_name, int argc, char *argv[]) {
...
static void *hypervisor_thread(void *arg) {
  ...
  while (conn->active) {
    ...
    if (res != 0) {
      ...
      hypervisor_exec_cmd(conn, tokens[0], tokens[1], ctx.tok_count - 2,
                          &tokens[2]);
...
static void sigpipe_handler(int sig) { printf("SIGPIPE received.\n"); }
...
int hypervisor_tcp_server(char *ip_addr, int tcp_port) {
  ...
  fd_count =
      ip_listen(ip_addr, tcp_port, SOCK_STREAM, HYPERVISOR_MAX_FD, fd_array);
  ...
  while (hypervisor_running) {
    ...
    for (i = 0; i < fd_count; i++) {
      ...
      if (!hypervisor_create_conn(clnt)) {
        fprintf(stderr,
                "hypervisor_tcp_server: unable to create new "
                "connection for FD %d\n",
                clnt);
        close(clnt);
      }
