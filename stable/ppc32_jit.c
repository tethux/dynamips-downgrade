...
int ppc32_jit_tcb_record_patch(ppc32_jit_tcb_t *block, jit_op_t *iop,
  ...
  if (vaddr & 0x03) {
    fprintf(stderr, "Block 0x%8.8x: trying to record an invalid IA (0x%8.8x)\n",
...
static int ppc32_jit_tcb_apply_patches(cpu_ppc_t *cpu, ppc32_jit_tcb_t *block,
                                       jit_op_t *iop) {
  struct ppc32_insn_patch *patch;
...
static int ppc32_op_gen_page(cpu_ppc_t *cpu, ppc32_jit_tcb_t *b) {
  ...
  for (b->ppc_trans_pos = 0; b->ppc_trans_pos < PPC32_INSN_PER_PAGE;
       b->ppc_trans_pos++) {
...
static inline ppc32_jit_tcb_t *ppc32_jit_tcb_compile(cpu_ppc_t *cpu,
                                                     m_uint32_t vaddr) {
  ppc32_jit_tcb_t *block;
