...
static void mips64_try_direct_far_jump(cpu_mips_t *cpu, mips64_jit_tcb_t *b,
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX, AMD64_R15,
                        OFFSET(cpu_mips_t, exec_blk_map), 8);
  ...
  /* Jump to the code */
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RSI, AMD64_RDX,
                        OFFSET(mips64_jit_tcb_t, jit_insn_ptr), 8);
...
static void mips64_emit_memop_fast64(mips64_jit_tcb_t *b, int write_op,
  ...
  if (write_op) {
    amd64_test_membase_imm_size(b->jit_ptr, AMD64_RCX,
                                OFFSET(mts64_entry_t, flags), MTS_FLAG_COW, 4);
    test2 = b->jit_ptr;
...
static void mips64_emit_memop_fast32(mips64_jit_tcb_t *b, int write_op,
  ...
  if (write_op) {
    amd64_test_membase_imm_size(b->jit_ptr, AMD64_RCX,
                                OFFSET(mts32_entry_t, flags), MTS_FLAG_COW, 4);
    test2 = b->jit_ptr;
    amd64_branch8(b->jit_ptr, X86_CC_NZ, 0, 1);
  }
...
void mips64_inc_cp0_count_reg(mips64_jit_tcb_t *b) {
  amd64_inc_membase(b->jit_ptr, AMD64_R15,
