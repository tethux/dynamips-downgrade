...
char *mips64_cp0_reg_names[MIPS64_CP0_REG_NR] = {
    "index",    "random",   "entry_lo0", "entry_lo1", "context",  "pagemask",
    "wired",    "info",     "badvaddr",  "count",     "entry_hi", "compare",
    "status",   "cause",    "epc",       "prid",      "config",   "ll_addr",
    "watch_lo", "watch_hi", "xcontext",  "cp0_r21",   "cp0_r22",  "cp0_r23",
    "cp0_r24",  "cp0_r25",  "ecc",       "cache_err", "tag_lo",   "tag_hi",
    "err_epc",  "cp0_r31",
};
...
static inline m_uint64_t mips64_cp0_get_reg_fast(cpu_mips_t *cpu,
                                                 u_int cp0_reg) {
  mips_cp0_t *cp0 = &cpu->cp0;
...
static inline void mips64_cp0_s1_set_reg(cpu_mips_t *cpu, u_int cp0_s1_reg,
  ...
  default:
    cpu_log(cpu->gen, "CP0_S1",
            "trying to set unknown register %u (val=0x%llx)\n", cp0_s1_reg,
            val);
  }
}
...
void mips64_tlb_dump_entry(cpu_mips_t *cpu, u_int index) {
  ...
  /* virtual Address */
  printf(" %2d: vaddr=0x%8.8llx ", index,
         entry->hi & mips64_cp0_get_vpn2_mask(cpu));
