...
void mips64_jit_create_ilt(void) {
  ...
  for (i = 0, count = 0; mips64_insn_tags[i].emit; i++) {
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&mips64_insn_tags[i].mask,
                          sizeof(mips64_insn_tags[i].mask));
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&mips64_insn_tags[i].value,
                          sizeof(mips64_insn_tags[i].value));
    count++;
  }

  ilt = ilt_create(
      "mips64j", crc32, count, (ilt_get_insn_cbk_t)mips64_jit_get_insn,
      (ilt_check_cbk_t)mips64_jit_chk_lo, (ilt_check_cbk_t)mips64_jit_chk_hi);
...
int mips64_jit_init(cpu_mips_t *cpu) {
  ...
  return (
      cpu_jit_init(cpu->gen, MIPS_JIT_VIRT_HASH_SIZE, MIPS_JIT_PHYS_HASH_SIZE));
}
...
/* Shutdown the JIT */
void mips64_jit_shutdown(cpu_mips_t *cpu) { cpu_jit_shutdown(cpu->gen); }
...
struct mips64_insn_tag *mips64_jit_fetch_and_emit(cpu_mips_t *cpu, cpu_tc_t *tc,
                                                  int delay_slot) {
  struct mips64_insn_tag *tag;
  mips_insn_t code;
...
int mips64_jit_tcb_record_patch(cpu_mips_t *cpu, cpu_tc_t *tc, u_char *jit_ptr,
                                m_uint64_t vaddr) {
...
static cpu_tb_t *mips64_jit_tcb_compile(cpu_mips_t *cpu, m_uint64_t vaddr,
...
static forced_inline void mips64_jit_tcb_run(cpu_mips_t *cpu, cpu_tb_t *tb) {
#if DEBUG_SYM_TREE
  ...
  if (cpu->sym_trace && cpu->sym_tree) {
    if ((sym = mips64_sym_lookup(cpu, cpu->pc)) != NULL) {
      cpu_log(cpu, "mips64_jit_tcb_run(start)",
              ...
              sym->name, cpu->pc, cpu->gpr[MIPS_GPR_RA], cpu->gpr[MIPS_GPR_A0],
              cpu->gpr[MIPS_GPR_A1], cpu->gpr[MIPS_GPR_A2],
              cpu->gpr[MIPS_GPR_A3]);
      mark = TRUE;
    }
  }
