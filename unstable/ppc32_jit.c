...
static void *ppc32_jit_get_insn(int index) { return (&ppc32_insn_tags[index]); }
...
void ppc32_jit_create_ilt(void) {
  ...
  for (i = 0, count = 0; ppc32_insn_tags[i].emit; i++) {
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&ppc32_insn_tags[i].mask,
                          sizeof(ppc32_insn_tags[i].mask));
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&ppc32_insn_tags[i].value,
                          sizeof(ppc32_insn_tags[i].value));
    count++;
  }

  ilt = ilt_create(
      "ppc32j", crc32, count, (ilt_get_insn_cbk_t)ppc32_jit_get_insn,
      (ilt_check_cbk_t)ppc32_jit_chk_lo, (ilt_check_cbk_t)ppc32_jit_chk_hi);
...
int ppc32_jit_init(cpu_ppc_t *cpu) {
  ...
  if (!cpu->exec_page_area) {
    fprintf(stderr, "ppc32_jit_init: unable to create exec area (size %lu)\n",
  ...
  cpu->exec_page_array = calloc(cpu->exec_page_count, sizeof(insn_exec_page_t));
  ...
  printf("CPU%u: carved JIT exec zone of %lu Mb into %lu pages of %u Kb.\n",
         cpu->gen->id, (u_long)(cpu->exec_page_area_size / 1048576),
...
int ppc32_jit_alloc_hreg_forced(cpu_ppc_t *cpu, int hreg) {
  ...
  if (cpu->hreg_map[hreg].flags != 0) {
    fprintf(stderr,
            "ppc32_alloc_hreg_forced: trying to force allocation "
            "of hreg %d (insn %s)\n",
...
int ppc32_jit_tcb_record_patch(ppc32_jit_tcb_t *tcb, jit_op_t *iop,
  ...
  if (vaddr & 0x03) {
    fprintf(stderr, "TCB 0x%8.8x: trying to record an invalid IA (0x%8.8x)\n",
            tcb->start_ia, vaddr);
    return (-1);
  }
...
static int ppc32_jit_tcb_apply_patches(cpu_ppc_t *cpu, ppc32_jit_tcb_t *tcb,
                                       jit_op_t *iop) {
  struct ppc32_insn_patch *patch;
...
static void ppc32_jit_tcb_free_code_chunks(cpu_ppc_t *cpu,
                                           ppc32_jit_tcb_t *tcb) {
  int i;
...
void ppc32_jit_tcb_free(cpu_ppc_t *cpu, ppc32_jit_tcb_t *tcb,
                        int list_removal) {
  if (tcb != NULL) {
    if (list_removal) {
...
static void ppc32_op_dump_opcode(jit_op_t *op) {
  ...
  case JIT_OP_LOAD_GPR:
    printf("load_gpr(%d,$%d,r:%d)", op->param[0], op->param[1], op->param[2]);
    break;
  case JIT_OP_STORE_GPR:
    printf("store_gpr(%d,$%d,r:%d)", op->param[0], op->param[1], op->param[2]);
    break;
  case JIT_OP_ALTER_HOST_REG:
    printf("alter_host_reg(%d)", op->param[0]);
    break;
  case JIT_OP_UPDATE_FLAGS:
    printf("update_flags(%d,%s)", op->param[0],
           (op->param[1] ? "signed" : "unsigned"));
    break;
...
_maybe_used static int ppc32_check_reg_map(ppc_reg_map_t *map_array,
                                           int *host_map) {
  ppc_reg_map_t *map;
  int i;
...
static int ppc32_op_gen_page(cpu_ppc_t *cpu, ppc32_jit_tcb_t *tcb) {
  ...
  for (tcb->ppc_trans_pos = 0; tcb->ppc_trans_pos < PPC32_INSN_PER_PAGE;
       tcb->ppc_trans_pos++) {
...
static ppc32_jit_tcb_t *ppc32_jit_tcb_compile(cpu_ppc_t *cpu, m_uint32_t vaddr,
                                              m_uint32_t exec_state) {
  ppc32_jit_tcb_t *tcb;
...
static forced_inline void ppc32_jit_tcb_run(cpu_ppc_t *cpu,
                                            ppc32_jit_tcb_t *tcb) {
...
void *ppc32_jit_run_cpu(cpu_gen_t *gen) {
  ...
  if (pthread_create(&timer_irq_thread, NULL, (void *)ppc32_timer_irq_run,
                     cpu)) {
