...
static void mips64_undef_access(cpu_mips_t *cpu, m_uint64_t vaddr,
  ...
  if (op_type == MTS_READ)
    cpu_log(cpu->gen, "MTS",
            "read  access to undefined address 0x%llx at "
            "pc=0x%llx (size=%u)\n",
            vaddr, cpu->pc, op_size);
  else
    cpu_log(cpu->gen, "MTS",
            "write access to undefined address 0x%llx at "
...
static void mips64_tlb_error(cpu_mips_t *cpu, m_uint64_t vaddr, int error,
  ...
  case MTS_ACC_T:
    ...
    if (op_code != MIPS_MEMOP_LOOKUP) {
      ...
      if (vaddr == 0) {
        cpu_log(cpu->gen, "MTS",
                "TLB exception suggests RELOAD for address 0x%llx at pc=0x%llx "
                "(%s access, size=%u)\n",
...
static void mips64_ae_error(cpu_mips_t *cpu, m_uint64_t vaddr, u_int op_code,
                            u_int op_type, u_int op_size) {
  if (op_code == MIPS_MEMOP_LOOKUP)
    return;

#if DEBUG_MTS_ACC_AE
  cpu_log(cpu->gen, "MTS",
          "AE exception for address 0x%llx at pc=0x%llx (%s access)\n", vaddr,
          cpu->pc, (op_type == MTS_READ) ? "read" : "write");
#endif
...
/* MTS64 slow lookup */
static mts64_entry_t *mips64_mts64_slow_lookup(cpu_mips_t *cpu,
                                               m_uint64_t vaddr, u_int op_code,
...
/* MTS64 access */
static forced_inline void *mips64_mts64_access(cpu_mips_t *cpu,
                                               m_uint64_t vaddr, u_int op_code,
                                               u_int op_size, u_int op_type,
                                               m_uint64_t *data) {
...
/* MTS32 slow lookup */
static mts32_entry_t *mips64_mts32_slow_lookup(cpu_mips_t *cpu,
                                               m_uint64_t vaddr, u_int op_code,
                                               u_int op_size, u_int op_type,
                                               m_uint64_t *data,
...
/* MTS32 access */
static forced_inline void *mips64_mts32_access(cpu_mips_t *cpu,
                                               m_uint64_t vaddr, u_int op_code,
                                               u_int op_size, u_int op_type,
                                               m_uint64_t *data) {
