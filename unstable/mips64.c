...
int mips64_cca_cached(m_uint8_t val) { return (cca_cache_status[val & 0x03]); }
...
void mips64_exec_syscall(cpu_mips_t *cpu) {
  ...
  printf("MIPS64: SYSCALL at PC=0x%llx (RA=0x%llx)\n"
         ...
         cpu->pc, cpu->gpr[MIPS_GPR_RA], cpu->gpr[MIPS_GPR_A0],
         cpu->gpr[MIPS_GPR_A1], cpu->gpr[MIPS_GPR_A2], cpu->gpr[MIPS_GPR_A3]);
#endif
...
void mips64_run_breakpoint(cpu_mips_t *cpu) {
  cpu_log(cpu->gen, "BREAKPOINT", "Virtual breakpoint reached at PC=0x%llx\n",
          cpu->pc);
...
void mips64_dump_regs(cpu_gen_t *cpu) {
  ...
  for (i = 0; i < MIPS64_GPR_NR / 2; i++) {
    printf("  %s ($%2d) = 0x%16.16llx   %s ($%2d) = 0x%16.16llx\n",
           ...
           mips64_gpr_reg_names[(i * 2) + 1], (i * 2) + 1,
           mcpu->gpr[(i * 2) + 1]);
  }
  ...
  for (i = 0; i < MIPS64_CP0_REG_NR / 2; i++) {
    printf("  %-10s ($%2d) = 0x%16.16llx   %-10s ($%2d) = 0x%16.16llx\n",
           mips64_cp0_reg_names[i * 2], i * 2, mips64_cp0_get_reg(mcpu, i * 2),
