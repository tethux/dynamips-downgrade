...
void vm_object_init(vm_obj_t *obj) { memset(obj, 0, sizeof(*obj)); }
...
/* Get VM type */
char *vm_get_type(vm_instance_t *vm) { return vm->platform->name; }
...
int vm_ram_init(vm_instance_t *vm, m_uint64_t paddr) {
  ...
  if (vm->ghost_status == VM_GHOST_RAM_USE) {
    return (dev_ram_ghost_init(vm, "ram", vm->sparse_mem,
...
int vm_init_vtty(vm_instance_t *vm) {
  ...
  vm->vtty_con =
      vtty_create(vm, "Console port", vm->vtty_con_type, vm->vtty_con_tcp_port,
                  &vm->vtty_con_serial_option);

  vm->vtty_aux =
      vtty_create(vm, "AUX port", vm->vtty_aux_type, vm->vtty_aux_tcp_port,
...
int vm_map_device(vm_instance_t *vm, struct vdevice *dev,
                  m_uint64_t base_addr) {
#if 0   
   /* Suspend VM activity */
   vm_suspend(vm);
...
static vm_ghost_image_t *vm_ghost_image_load(char *filename) {
  ...
  img->fd =
      memzone_open_file_ro(img->filename, &img->area_ptr, &img->file_size);
...
int vm_ghost_image_release(int fd) {
  ...
  for (img = &vm_ghost_pool; *img; img = &(*img)->next) {
    if ((*img)->fd == fd) {
      ...
      if ((*img)->ref_count == 0) {
        m_log("GHOST",
              "unloaded ghost image %s (fd=%d) at "
              "addr=%p (size=0x%llx)\n",
...
int vm_mmap_open_file(vm_instance_t *vm, char *name, u_char **ptr,
                      off_t *fsize) {
  char *filename;
  int fd;
...
int vm_mmap_create_file(vm_instance_t *vm, char *name, size_t len,
                        u_char **ptr) {
  char *filename;
  int fd;
...
int vm_ios_set_config(vm_instance_t *vm, const char *startup_filename,
...
int vm_nvram_extract_config(vm_instance_t *vm, char *filename) {
  ...
  if ((vm->platform->nvram_extract_config(vm, &cfg_buffer, &cfg_len, NULL,
                                          NULL)) ||
      (cfg_buffer == NULL))
    return (-1);
...
/* Read IOS configuraton from the files and push it to NVRAM (NULL to keep
 * existing data) */
int vm_nvram_push_config(vm_instance_t *vm, const char *startup_filename,
                         const char *private_filename) {
  ...
  /* Push it! */
  res = vm->platform->nvram_push_config(vm, startup_config, startup_len,
...
int vm_delete_instance(char *name) {
  return (registry_delete_if_unused(name, OBJ_TYPE_VM, vm_reg_delete_instance,
                                    NULL));
}
...
int vm_rename_instance(vm_instance_t *vm, char *name) {
  ...
  if (do_rename) {
    ...
    for (i = 0; i < globbuf.gl_pathc; i++) {
      if ((filename = dyn_sprintf("%s_%s_%s", vm_get_type(vm), vm->name,
                                  globbuf.gl_pathv[i] + strlen(pattern) - 1)) ==
          NULL)
        break; /* out of memory */

      if (rename(globbuf.gl_pathv[i], filename) != 0) {
        fprintf(stderr,
                "Warning: vm_rename_instance: rename(\"%s\",\"%s\"): %s\n",
