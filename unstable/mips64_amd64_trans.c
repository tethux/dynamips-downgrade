...
void mips64_set_ra(cpu_tc_t *b, m_uint64_t ret_pc) {
  ...
  amd64_mov_membase_reg(b->jit_ptr, AMD64_R15, REG_OFFSET(MIPS_GPR_RA),
                        AMD64_RAX, 8);
}
...
static void mips64_try_direct_far_jump(cpu_mips_t *cpu, cpu_tc_t *b,
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RSI, AMD64_R15,
                        OFFSET(cpu_mips_t, gen), 8);

  /* Get JIT block info in %rdx */
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX, AMD64_RSI,
                        OFFSET(cpu_gen_t, tb_virt_hash), 8);
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RDX, AMD64_RBX,
                        pc_hash * sizeof(void *), 8);
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX, AMD64_RDX, OFFSET(cpu_tb_t, tc),
                        8);
  ...
  /* Jump to the code */
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RSI, AMD64_RBX,
                        OFFSET(cpu_tc_t, jit_insn_ptr), 8);
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RAX, AMD64_RSI,
                        pc_offset * sizeof(void *), 8);
...
static void mips64_memop_fast_lw(cpu_tc_t *b, int target) {
  ...
  amd64_mov_membase_reg(b->jit_ptr, AMD64_R15, REG_OFFSET(target), AMD64_RDX,
                        8);
}
...
static void mips64_memop_fast_sw(cpu_tc_t *b, int target) {
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RAX, AMD64_R15, REG_OFFSET(target),
                        4);
  amd64_bswap32(b->jit_ptr, X86_EAX);
...
static void mips64_emit_memop_fast64(cpu_tc_t *b, int write_op, int opcode,
  ...
  amd64_alu_reg_membase(b->jit_ptr, X86_ADD, AMD64_RSI, AMD64_R15,
                        REG_OFFSET(base));
  ...
  /* RCX = mts64 entry */
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RCX, AMD64_R15,
  ...
  if (write_op) {
    amd64_test_membase_imm_size(b->jit_ptr, AMD64_RCX,
                                OFFSET(mts64_entry_t, flags), MTS_FLAG_WRCATCH,
                                4);
    test2 = b->jit_ptr;
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RBX, AMD64_RCX,
                        OFFSET(mts64_entry_t, hpa), 8);
...
static void mips64_emit_memop_fast32(cpu_tc_t *b, int write_op, int opcode,
  ...
  amd64_alu_reg_membase_size(b->jit_ptr, X86_ADD, X86_ESI, AMD64_R15,
                             REG_OFFSET(base), 4);
  ...
  if (write_op) {
    amd64_test_membase_imm_size(b->jit_ptr, AMD64_RCX,
                                OFFSET(mts32_entry_t, flags), MTS_FLAG_WRCATCH,
                                4);
    test2 = b->jit_ptr;
    amd64_branch8(b->jit_ptr, X86_CC_NZ, 0, 1);
  }
...
static void mips64_emit_memop(cpu_tc_t *b, int op, int base, int offset,
  ...
  /* RSI = GPR[base] + sign-extended offset */
  mips64_load_imm(b, AMD64_RSI, val);
  amd64_alu_reg_membase(b->jit_ptr, X86_ADD, AMD64_RSI, AMD64_RDI,
                        REG_OFFSET(base));
...
void mips64_inc_cp0_count_reg(cpu_tc_t *b) {
  amd64_inc_membase_size(b->jit_ptr, AMD64_R15,
                         OFFSET(cpu_mips_t, cp0_virt_cnt_reg), 4);
...
void mips64_check_pending_irq(cpu_tc_t *b) {
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RAX, AMD64_R15,
...
void mips64_inc_perf_counter(cpu_tc_t *b) {
  amd64_inc_membase_size(b->jit_ptr, AMD64_R15,
                         OFFSET(cpu_mips_t, perf_counter), 4);
}
...
DECLARE_INSN(BEQ) {
   ...
   */
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RAX, AMD64_R15, REG_OFFSET(rs), 8);
  amd64_alu_reg_membase(b->jit_ptr, X86_CMP, AMD64_RAX, AMD64_R15,
                        REG_OFFSET(rt));
  test1 = b->jit_ptr;
...
DECLARE_INSN(DADDIU) {
  ...
  amd64_alu_reg_membase(b->jit_ptr, X86_ADD, AMD64_RCX, AMD64_R15,
                        REG_OFFSET(rs));
...
DECLARE_INSN(MFHI) {
  ...
  amd64_mov_reg_membase(b->jit_ptr, AMD64_RDX, AMD64_R15,
                        OFFSET(cpu_mips_t, hi), 8);
