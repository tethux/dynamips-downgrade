...
#define ppc_emit_jump_code(code, target, lk)                                   \
  do {                                                                         \
                                                                               \
    ...
    if ((target) - (code) >= 0) {                                              \
      ...
      if ((target) - (code) >= -33554432) {                                    \
        ppc_emit32((code),                                                     \
                   (18 << 26) | (((target) - (code)) & ~0xfc000000) | lk);     \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
                                                                               \
    if ((long)(target) >= 0) {                                                 \
      ...
      if ((long)(target) >= -33554432) {                                       \
        ppc_emit32((code), (18 << 26) |                                        \
                               ((unsigned int)(target) & ~0xfc000000) | 2 |    \
                               lk);                                            \
        break;                                                                 \
      }                                                                        \
    }                                                                          \
...
static forced_inline void mips64_jit_tcb_push_epilog(cpu_tc_t *tc) {
  ...
  ppc_lwz(tc->jit_ptr, ppc_r0, PPC_STACK_DECREMENTER + PPC_RET_ADDR_OFFSET,
          ppc_r1);
...
static forced_inline void mips64_jit_tcb_exec(cpu_mips_t *cpu, cpu_tb_t *tb) {
  ...
  __asm__ __volatile__("mtlr   r12                   \n"
                       ...
                       "lwz     r1,    0(r1)         \n"
                       : "+r"(jit_code)
                       : "r"(cpu), "i"(PPC_STACK_PARAM_OFFSET),
                         "i"(PPC_RET_ADDR_OFFSET), "i"(-PPC_STACK_DECREMENTER)
                       : "r0", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10",
                         "r11", /*"r12",*/ "lr", "ctr", "xer", "cr0", "cr1",
                         "cr5", "cr6", "cr7", "memory");
}
