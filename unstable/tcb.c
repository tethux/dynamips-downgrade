 ...
 */
static inline void tc_remove_cpu_local(cpu_tc_t *tc) { M_LIST_REMOVE(tc, sc); }
...
static forced_inline int tb_compare(cpu_tb_t *tb1, cpu_tb_t *tb2) {
  return ((tb1->vaddr == tb2->vaddr) && (tb1->exec_state == tb2->exec_state));
}
  ...
  for (i = 0; i < TC_HASH_SIZE; i++) {
    for (tc = tsg->tc_hash[i]; tc; tc = tc->hash_next) {
      if (!(tc->flags & TC_FLAG_VALID)) {
        cpu_log(cpu, "JIT", "consistency error: TC 0x%8.8llx (flags=0x%x)\n",
                tc->vaddr, tc->flags);
        tc_dump(tc);
        err++;
      }
      ...
      for (tb = tc->tb_list; tb; tb = tb->tb_dl_next) {
        if (!(tb->flags & TB_FLAG_VALID)) {
          cpu_log(cpu, "JIT", "consistency error: TB 0x%8.8llx (flags=0x%x)\n",
                  tb->vaddr, tb->flags);
          err++;
...
void tsg_show_stats(void) {
  ...
  printf("  ID   Shared TC     Total TC     Alloc.Pages  Shared Pages   Total "
         "Pages\n");

  for (i = 0; i < TSG_MAX_GROUPS; i++) {
    if (!tsg_get_stats(tsg_array[i], &s)) {
      printf(" %3d     %8u     %8u       %8u      %8u      %8u\n", i,
             s.shared_tc, s.total_tc, tsg_array[i]->exec_page_alloc,
             s.shared_pages, tsg_array[i]->exec_page_total);
    }
  }
...
/* Apply all patches */
int tc_apply_patches(cpu_tc_t *tc,
                     void (*set_patch)(u_char *insn, u_char *dst)) {
...
void cpu_jit_write_on_exec_page(cpu_gen_t *cpu, m_uint32_t wr_phys_page,
                                m_uint32_t wr_hp, m_uint32_t ip_phys_page) {
