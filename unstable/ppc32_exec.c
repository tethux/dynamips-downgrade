...
void ppc32_exec_create_ilt(void) {
  ...
  for (i = 0, count = 0; ppc32_exec_tags[i].exec; i++) {
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&ppc32_exec_tags[i].mask,
                          sizeof(ppc32_exec_tags[i].mask));
    crc32 = crc32_compute(~crc32, (m_uint8_t *)&ppc32_exec_tags[i].value,
                          sizeof(ppc32_exec_tags[i].value));
    count++;
  }

  ilt = ilt_create(
      "ppc32e", crc32, count, (ilt_get_insn_cbk_t)ppc32_exec_get_insn,
      (ilt_check_cbk_t)ppc32_exec_chk_lo, (ilt_check_cbk_t)ppc32_exec_chk_hi);
...
void ppc32_dump_stats(cpu_ppc_t *cpu) {
  ...
  for (i = 0; ppc32_exec_tags[i].exec; i++)
    printf("  * %-10s : %10llu\n", ppc32_exec_tags[i].name,
           ppc32_exec_tags[i].count);
...
static forced_inline int ppc32_exec_single_instruction(cpu_ppc_t *cpu,
                                                       ppc_insn_t instruction) {
...
/* Update CR0 */
static forced_inline void ppc32_exec_update_cr0(cpu_ppc_t *cpu,
                                                m_uint32_t val) {
  m_uint32_t res;
...
static int ppc32_exec_EIEIO(cpu_ppc_t *cpu, ppc_insn_t insn) { return (0); }
...
static int ppc32_exec_ISYNC(cpu_ppc_t *cpu, ppc_insn_t insn) { return (0); }
...
/* SYNC - Synchronize */
static int ppc32_exec_SYNC(cpu_ppc_t *cpu, ppc_insn_t insn) { return (0); }
...
static int ppc32_exec_TLBSYNC(cpu_ppc_t *cpu, ppc_insn_t insn) { return (0); }
